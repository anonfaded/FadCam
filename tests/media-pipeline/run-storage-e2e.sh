...
ready=0
for i in {1..60}; do
  if curl -fsS "${GATEWAY_URL}/health" >/dev/null && curl -fsS "${CORE_URL}/api/v1/health" >/dev/null && curl -fsS -u "${API_USER}:${API_PASSWORD}" "${API_URL}/v3/paths/list" >/dev/null && curl -fsS http://localhost:9000/minio/health/live >/dev/null && mc ls e2e >/dev/null 2>&1; then ready=1; break; fi
  sleep 1
done
...
# Authentication contract: invalid and expired credentials must fail closed.
...
# Authorization contract: registration, duplicate conflict, ownership isolation and permissions.
