...
Every validation run that successfully builds an APK publishes the binary as a GitHub Actions artifact attached to that exact run. This means each generated APK is traceable to:

```text
commit → workflow run → build job → APK artifact
```

The artifact is intentionally tied to the commit that produced it. Do **not** treat an APK from an older run as equivalent to the current `master` build.
...
| --- | --- | --- |
| `fadcam-default-debug-apk` | `DefaultDebug` | Development, smoke testing, and device installation. |
| `fadcam-default-release-apk` | `DefaultRelease` | Release-build validation and installation testing. |

### Build record to preserve

When recording a build for testing or reporting, preserve these four values:
...
Artifact: fadcam-default-release-apk
Variant: DefaultRelease
```

This prevents confusion when several APKs have been generated in a short period.

### Important retention rule

CI artifacts are temporary. The workflow currently retains APK artifacts for **7 days**. A successful build therefore does **not** automatically become a permanent release.
...
For every APK generated by the engineering/release-validation pipeline, go to:
...
1. Open the Actions workflow.
2. Select the successful run for the commit you want.
3. Confirm the commit SHA/run before downloading.
4. Scroll to **Artifacts**.
5. Download the required artifact.
6. Extract the ZIP.
7. Install the APK on the Android test device/emulator.
...
The release artifact is CI-signed with a short-lived validation key. It is suitable for validation, **not** as the project's permanent production APK.

### Actions artifacts vs permanent releases

| | Actions artifact | GitHub Release APK |
| --- | --- | --- |
| Best for | Commit/PR testing | User distribution |
| Traceability | Workflow run + commit | Version/tag |
| Retention | Temporary | Permanent until removed |
| Signing | CI validation key | Production release key |
| Recommended for users | No | Yes |
...
8. Record the commit SHA and workflow run for every APK used in device testing.
