/**
 * ServaCamFragLoader — HLS.js custom fragment loader with AES-256-GCM decryption.
 *
 * Intercepts HLS media segment (*.m4s) downloads and transparently decrypts
 * ServaCam E2E-encrypted payloads before passing the cleartext to HLS.js.
 *
 ...
 *  - window.ServaCamRemote must be available when a stream is active.
...
(function () {
  ...
  /**
   * Check the first 4 bytes of |buf| for the ServaCam magic.
  ...
  function buildLoader() {
    if (typeof Hls === 'undefined') {
      console.error('[ServaCamFragLoader] HLS.js is not loaded — cannot create loader');
      return null;
    }

    const DefaultLoader = Hls.DefaultConfig.loader;

    class ServaCamFragLoader extends DefaultLoader {
      /**
        ...
        const patchedCallbacks = Object.assign({}, callbacks, {
          onSuccess: async (response, stats, ctx, networkDetails) => {
            try {
              ...
              console.error('[ServaCamFragLoader] Decryption error:', err.message);
  ...
  /**
   * Get the current stream device ID from ServaCamRemote (cloud mode).
   * @returns {string|null}
   */
  function getStreamDeviceId() {
    if (typeof ServaCamRemote !== 'undefined' && typeof ServaCamRemote.getStreamDeviceId === 'function') {
      return ServaCamRemote.getStreamDeviceId();
    }
    return null;
  }
  ...
  if (loader) {
    window.ServaCamFragLoader = loader;
    console.log('[ServaCamFragLoader] Loaded and ready');
  }
