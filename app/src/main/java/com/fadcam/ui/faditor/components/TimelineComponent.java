...
public class TimelineComponent extends View {
    ...
    private void initializeGestureDetectors() {
        gestureDetector = new GestureDetector(getContext(), new GestureDetector.SimpleOnGestureListener() {
            ...
            public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
                if (!isDraggingStartHandle && !isDraggingEndHandle && !isDraggingPlayhead) {
    ...
    public void zoomToFit() {
        if (videoDuration > 0) {
            ...
            float optimalZoom = availableWidth / (float) videoDuration * 1000f; // Convert to pixels per second
    ...
    private void drawTracks(Canvas canvas) {
        ...
        for (int i = 0; i < tracks.size(); i++) {
            ...
            // Track background
            trackPaint.setColor(i == selectedTrackIndex ? COLOR_TRACK_BG : (COLOR_TRACK_BG & 0x80FFFFFF));
            canvas.drawRect(0, currentY, getWidth(), currentY + trackHeight, trackPaint);
            
            // Track label
            textPaint.setTextAlign(Paint.Align.LEFT);
            canvas.drawText(track.getName(), dpToPx(8), currentY + trackHeight / 2f, textPaint);
    ...
    private void drawFrameMarkers(Canvas canvas, float pixelsPerMs) {
        ...
        if (framePixelWidth > 2f) { // Only draw if frames are visible
            ...
            for (long frame = startFrame; frame <= endFrame; frame++) {
                ...
                if (x >= 0 && x <= getWidth()) {
                    canvas.drawLine(x, getHeight() - dpToPx(20), x, getHeight(), framePaint);
                }
            }
    ...
    private void drawWaveform(Canvas canvas) {
        ...
        for (int i = 0; i < waveformData.length - 1; i++) {
            ...
            // Draw waveform line
            canvas.drawLine(x1, waveformCenterY - amplitude1, x2, waveformCenterY - amplitude2, waveformPaint);
            canvas.drawLine(x1, waveformCenterY + amplitude1, x2, waveformCenterY + amplitude2, waveformPaint);
        }
    }
    ...
    private void drawTrimRange(Canvas canvas) {
        ...
        // Start handle
        tempRectF.set(startX - handleWidth / 2f, 0, startX + handleWidth / 2f, getHeight());
        canvas.drawRect(tempRectF, handlePaint);
        
        // End handle
        tempRectF.set(endX - handleWidth / 2f, 0, endX + handleWidth / 2f, getHeight());
        canvas.drawRect(tempRectF, handlePaint);
    ...
    private void drawZoomControls(Canvas canvas) {
        ...
        // Zoom out button
        tempRectF.set(getWidth() - margin - controlSize * 2 - dpToPx(4), margin, 
                     getWidth() - margin - controlSize - dpToPx(4), margin + controlSize);
        canvas.drawRoundRect(tempRectF, dpToPx(4), dpToPx(4), zoomControlPaint);
        canvas.drawText("-", tempRectF.centerX(), tempRectF.centerY() + dpToPx(4), textPaint);
        
        // Zoom in button
        tempRectF.set(getWidth() - margin - controlSize, margin, 
                     getWidth() - margin, margin + controlSize);
        canvas.drawRoundRect(tempRectF, dpToPx(4), dpToPx(4), zoomControlPaint);
        canvas.drawText("+", tempRectF.centerX(), tempRectF.centerY() + dpToPx(4), textPaint);
    }
    ...
    private boolean handleTouchUp(float x, float y) {
        boolean wasHandled = isDraggingStartHandle || isDraggingEndHandle || isDraggingPlayhead;
    ...
    private boolean handleZoomControlTouch(float x, float y) {
        ...
        // Zoom out button
        if (x >= getWidth() - margin - controlSize * 2 - dpToPx(4) && 
            x <= getWidth() - margin - controlSize - dpToPx(4) &&
            y >= margin && y <= margin + controlSize) {
            zoomOut();
            return true;
        }
        
        // Zoom in button
        if (x >= getWidth() - margin - controlSize && 
            x <= getWidth() - margin &&
            y >= margin && y <= margin + controlSize) {
            zoomIn();
            return true;
    ...
    private void scrollTimeline(float distanceX) {
        ...
        long newViewportEnd = Math.min(videoDuration, viewportEnd + scrollAmount);
    ...
    public boolean isFrameSnappingEnabled() { return frameSnappingEnabled; }
    public boolean isScrubbing() { return isScrubbing; }
