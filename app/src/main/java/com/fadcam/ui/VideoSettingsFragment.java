 ...
 * Row based extraction of video recording related settings from monolithic SettingsFragment.
...
public class VideoSettingsFragment extends Fragment {
    ...
    private void refreshAllValues(){
        ...
        valueCamera.setText(cam == CameraType.FRONT ? getString(R.string.button_settings_cam_front) : getString(R.string.button_settings_cam_back));
        ...
        if (cam == CameraType.BACK) {
            ...
            String display = null;
            if(savedId!=null){
                for(CameraIdInfo info: availableBackCameras){ if(info.id.equals(savedId)){ display = info.displayName; break; } }
            }
        ...
        // Resolution (string builder reused from legacy approach)
        Size res = prefs.getCameraResolution();
        valueResolution.setText(res.getWidth() + "x" + res.getHeight());
        ...
        valueBitrate.setText(String.format(Locale.getDefault(), "%d Mbps %s", bitrate/1000, isCustom?"(Custom)":"(Auto)"));
    // Bitrate helper removed from persistent UI
        ...
        if(valueZoomRatio!=null){
            ...
            if(zrRow!=null) zrRow.setVisibility(View.VISIBLE);
        }
        ...
        if(valueVideoSplitEnabled!=null){
            ...
            if(enabled){
                ...
                String label;
                if(mb==500) label = "Enabled (500 MB)";
                else if(mb==1024) label = "Enabled (1 GB)";
                else if(mb==2048) label = "Enabled (2 GB)";
                else if(mb==4096) label = "Enabled (4 GB)";
                else label = "Enabled (Custom " + mb + " MB)";
    ...
    private void showCameraBottomSheet(){
        ...
        items.add(new com.fadcam.ui.picker.OptionItem(CameraType.FRONT.toString(), getString(R.string.button_settings_cam_front)));
        items.add(new com.fadcam.ui.picker.OptionItem(CameraType.BACK.toString(), getString(R.string.button_settings_cam_back)));
    ...
    private void showLensBottomSheet(){
        ...
        if(prefs.getCameraSelection()==CameraType.FRONT){ return; }
        if(availableBackCameras.isEmpty()) detectAvailableBackCameras();
        String saved = prefs.getSelectedBackCameraId();
        java.util.ArrayList<com.fadcam.ui.picker.OptionItem> items = new java.util.ArrayList<>();
        for(CameraIdInfo info: availableBackCameras){ items.add(new com.fadcam.ui.picker.OptionItem(info.id, info.displayName)); }
    ...
    private void showResolutionBottomSheet(){
        // -------------- Fix Start for this method(showResolutionBottomSheet)-----------
        CameraType cam = prefs.getCameraSelection();
    ...
    com.fadcam.ui.picker.PickerBottomSheetFragment sheet = com.fadcam.ui.picker.PickerBottomSheetFragment.newInstance(
        ...
        // -------------- Fix Ended for this method(showResolutionBottomSheet)-----------
    }
    ...
    private void showFrameRateBottomSheet(){
        ...
        getParentFragmentManager().setFragmentResultListener(resultKey, this, (key,bundle)->{
            String sel = bundle.getString(com.fadcam.ui.picker.PickerBottomSheetFragment.BUNDLE_SELECTED_ID);
            if(sel!=null && !sel.equals(currentId)){
                try{ int fps = Integer.parseInt(sel); if(fps!=current){ prefs.setSpecificVideoFrameRate(cam, fps); onResolutionOrFramerateChanged(); } }catch(Exception ignored){}
                refreshAllValues();
            }
        });
    ...
    private void showCodecBottomSheet(){
        ...
        for(VideoCodec c: codes){ items.add(new com.fadcam.ui.picker.OptionItem(c.toString(), c.toString())); }
    ...
    private void showBitrateBottomSheet(){
        ...
        getParentFragmentManager().setFragmentResultListener(resultKey, this, (k,b)->{
            ...
            if(sel==null) return;
            if("auto".equals(sel)){
                setBitrateMode(false); setBitrateCustomValue(getDefaultBitrate()); onResolutionOrFramerateChanged(); refreshAllValues();
    ...
    com.fadcam.ui.picker.PickerBottomSheetFragment sheet = com.fadcam.ui.picker.PickerBottomSheetFragment.newInstance(
        getString(R.string.setting_video_bitrate_title), items, custom?"custom":"auto", resultKey, helper);
    ...
    private void showBitrateCustomInput(){
        ...
        getParentFragmentManager().setFragmentResultListener(resultKey, this, (k,b)->{
            if(b.containsKey(com.fadcam.ui.picker.NumberInputBottomSheetFragment.RESULT_NUMBER)){
                ...
                setBitrateCustomValue(val*1000); setBitrateMode(true); onResolutionOrFramerateChanged(); refreshAllValues();
            }
        });
    com.fadcam.ui.picker.NumberInputBottomSheetFragment sheet = com.fadcam.ui.picker.NumberInputBottomSheetFragment.newInstance(
        ...
        getString(R.string.bitrate_info_warning_low), getString(R.string.bitrate_info_warning_high), resultKey);
        sheet.show(getParentFragmentManager(), "bitrate_value_input");
        // -------------- Fix Ended for this method(showBitrateCustomInput)-----------
    }

    // Removed legacy orientation dialog + feature flag wrapper; always show bottom sheet picker

    private void showOrientationBottomSheet(){
        // -------------- Fix Start for this method(showOrientationBottomSheet)-----------
        ArrayList<com.fadcam.ui.picker.OptionItem> items = new ArrayList<>();
        items.add(new com.fadcam.ui.picker.OptionItem(SharedPreferencesManager.ORIENTATION_PORTRAIT, getString(R.string.video_orientation_portrait)));
        items.add(new com.fadcam.ui.picker.OptionItem(SharedPreferencesManager.ORIENTATION_LANDSCAPE, getString(R.string.video_orientation_landscape)));
    ...
    com.fadcam.ui.picker.PickerBottomSheetFragment sheet = com.fadcam.ui.picker.PickerBottomSheetFragment.newInstance(
        ...
        // -------------- Fix Ended for this method(showOrientationBottomSheet)-----------
    }

    // -------- Legacy logic helpers (migrated/adapted) --------

    private void detectAvailableBackCameras(){
        // -------------- Fix Start for this method(detectAvailableBackCameras)-----------
        ...
        try{
            ...
            for(String id: manager.getCameraIdList()){
                ...
                if(facing!=null && facing == CameraCharacteristics.LENS_FACING_BACK){
                    ...
                    Float focal = null; if(focalLengths!=null && focalLengths.length>0) focal = focalLengths[0];
                    StringBuilder display = new StringBuilder();
                    boolean isDefault = id.equals(defaultId);
                    if(isDefault){ display.append("Main"); }
                    else if(focal!=null){
                        if(focal <= 4f) display.append("Ultra-Wide");
                        else if(focal <= 8f) display.append("Wide-Angle");
                        else if(focal >= 50f) display.append("Telephoto");
                        else if(focal >= 35f) display.append("Portrait");
                        else display.append("Camera");
                    } else { display.append("Camera"); }
                    display.append(" (").append(id).append(")");
                    if(focal!=null) display.append(" ").append(Math.round(focal)).append("mm");
            ...
            java.util.Collections.sort(availableBackCameras, (a,b)->{
                if(a.id.equals(defaultId)) return -1; if(b.id.equals(defaultId)) return 1; try{ return Integer.parseInt(a.id)-Integer.parseInt(b.id);}catch(Exception e){ return a.id.compareTo(b.id);} });
            if(availableBackCameras.isEmpty()){
                availableBackCameras.add(new CameraIdInfo(defaultId, "Default Camera ("+defaultId+")"));
            }
            String saved = prefs.getSelectedBackCameraId();
            boolean found=false; for(CameraIdInfo info: availableBackCameras){ if(info.id.equals(saved)){ found=true; break;} }
            if(!found){ prefs.setSelectedBackCameraId(availableBackCameras.get(0).id); }
        }catch(Exception e){ Log.e(TAG, "Error detecting back cameras", e); }
        // -------------- Fix Ended for this method(detectAvailableBackCameras)-----------
    }
    ...
    private List<CamcorderProfile> getCamcorderProfilesForTypeInternal(CameraType cameraType){
        Context ctx = getContext(); if(ctx==null) return new ArrayList<>();
        String actualCameraId = getActualCameraIdForType(cameraType); if(actualCameraId==null) return new ArrayList<>();
        List<Size> supportedVideoSizes = getSupportedVideoSizesForCamera(actualCameraId);
        if(supportedVideoSizes.isEmpty()) return getCamcorderProfilesFallback(cameraType);
        List<CamcorderProfile> profiles = new ArrayList<>();
        for(Size s: supportedVideoSizes){ CamcorderProfile p = createProfileForSize(actualCameraId, s); if(p!=null) profiles.add(p); }
    ...
    private List<Size> getSupportedVideoSizesForCamera(String cameraId){
        ...
        try{
            Context ctx = getContext(); if(ctx==null) return supportedSizes;
            ...
            if(configMap!=null){ Size[] videoSizes = configMap.getOutputSizes(MediaRecorder.class); if(videoSizes!=null){ Arrays.sort(videoSizes,(s1,s2)-> Long.compare((long)s2.getWidth()*s2.getHeight(), (long)s1.getWidth()*s1.getHeight())); for(Size size: videoSizes){ if(isReasonableVideoSize(size)) supportedSizes.add(size); } } }
        }catch(Exception e){ Log.e(TAG, "Error accessing camera "+cameraId+" for video sizes", e); }
    ...
    private CamcorderProfile createProfileForSize(String cameraId, Size size){
        try{
            int[] qualities = { CamcorderProfile.QUALITY_2160P, CamcorderProfile.QUALITY_1080P, CamcorderProfile.QUALITY_720P, CamcorderProfile.QUALITY_480P, CamcorderProfile.QUALITY_HIGH, CamcorderProfile.QUALITY_LOW };
            if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.S){ int[] ext = { CamcorderProfile.QUALITY_8KUHD, CamcorderProfile.QUALITY_2160P, CamcorderProfile.QUALITY_1080P, CamcorderProfile.QUALITY_720P, CamcorderProfile.QUALITY_480P, CamcorderProfile.QUALITY_HIGH, CamcorderProfile.QUALITY_LOW }; qualities = ext; }
            int camIdInt = Integer.parseInt(cameraId);
            for(int q: qualities){ if(CamcorderProfile.hasProfile(camIdInt,q)){ CamcorderProfile p = CamcorderProfile.get(camIdInt,q); if(p!=null && p.videoFrameWidth== size.getWidth() && p.videoFrameHeight== size.getHeight()) return p; } }
        }catch(Exception e){ Log.e(TAG,"createProfileForSize error", e); }
        return null;
    }
    ...
    private List<CamcorderProfile> getCamcorderProfilesFallback(CameraType cameraType){
        ...
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.S && CamcorderProfile.hasProfile(cameraId, CamcorderProfile.QUALITY_8KUHD)) profiles.add(CamcorderProfile.get(cameraId, CamcorderProfile.QUALITY_8KUHD));
        if(CamcorderProfile.hasProfile(cameraId, CamcorderProfile.QUALITY_2160P)) profiles.add(CamcorderProfile.get(cameraId, CamcorderProfile.QUALITY_2160P));
        if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.R && CamcorderProfile.hasProfile(cameraId, CamcorderProfile.QUALITY_2K)) profiles.add(CamcorderProfile.get(cameraId, CamcorderProfile.QUALITY_2K));
        int[] qualities = { CamcorderProfile.QUALITY_1080P, CamcorderProfile.QUALITY_720P, CamcorderProfile.QUALITY_480P, CamcorderProfile.QUALITY_CIF, CamcorderProfile.QUALITY_QCIF, CamcorderProfile.QUALITY_LOW };
        Set<Integer> added = new HashSet<>(); for(CamcorderProfile p: profiles){ if(p!=null) added.add(p.quality); }
        for(int q: qualities){ if(!added.contains(q) && CamcorderProfile.hasProfile(cameraId,q)){ CamcorderProfile p = CamcorderProfile.get(cameraId,q); if(p!=null){ profiles.add(p); added.add(q);} } }
        if(!added.contains(CamcorderProfile.QUALITY_HIGH) && CamcorderProfile.hasProfile(cameraId, CamcorderProfile.QUALITY_HIGH)) profiles.add(CamcorderProfile.get(cameraId, CamcorderProfile.QUALITY_HIGH));
        profiles.removeIf(Objects::isNull);
        return profiles;
    }
    ...
    private String buildResolutionLabel(Size size){
        ...
        String label = null;
        for(int i=0;i<keys.length;i++){ if(keys[i].equals(resKey)){ label = values[i]; break; } }
        if(label==null) return resKey; // fallback
        return label + " ("+resKey+")";
    }
    ...
    private String getActualCameraIdForType(CameraType type){
        ...
        CameraManager manager = (CameraManager) ctx.getSystemService(Context.CAMERA_SERVICE);
        try{ for(String id: manager.getCameraIdList()){ CameraCharacteristics ch = manager.getCameraCharacteristics(id); Integer facing = ch.get(CameraCharacteristics.LENS_FACING); if(facing!=null){ if(type==CameraType.FRONT && facing==CameraCharacteristics.LENS_FACING_FRONT) return id; if(type==CameraType.BACK && facing==CameraCharacteristics.LENS_FACING_BACK){ return id; } } } }catch(Exception e){ Log.e(TAG, "Error resolving camera id", e);} return null; }

    private List<Integer> getHardwareSupportedFrameRates(CameraType cameraType){
        // Derived from legacy method snippet
        Context ctx = getContext(); if(ctx==null) return new ArrayList<>();
        List<Integer> defaultList = new ArrayList<>(); defaultList.add(Constants.DEFAULT_VIDEO_FRAME_RATE);
        CameraManager manager = (CameraManager) ctx.getSystemService(Context.CAMERA_SERVICE);
        String targetCameraId = getActualCameraIdForType(cameraType);
        if(targetCameraId==null) return defaultList;
        Range<Integer>[] ranges = null;
        try{ CameraCharacteristics ch = manager.getCameraCharacteristics(targetCameraId); ranges = ch.get(CameraCharacteristics.CONTROL_AE_AVAILABLE_TARGET_FPS_RANGES); }catch(Exception e){ Log.e(TAG, "FPS ranges error", e); return defaultList; }
        Set<Integer> framerates = new TreeSet<>(); int maxProfileFps = 30; try{ int camIdInt = Integer.parseInt(targetCameraId); int[] qualities = {CamcorderProfile.QUALITY_HIGH, CamcorderProfile.QUALITY_2160P, CamcorderProfile.QUALITY_1080P, CamcorderProfile.QUALITY_720P}; for(int q: qualities){ if(CamcorderProfile.hasProfile(camIdInt,q)){ CamcorderProfile p = CamcorderProfile.get(camIdInt,q); if(p!=null && p.videoFrameRate> maxProfileFps) maxProfileFps = p.videoFrameRate; } } if(Build.VERSION.SDK_INT>=29){ int[] high = { CamcorderProfile.QUALITY_HIGH_SPEED_HIGH, CamcorderProfile.QUALITY_HIGH_SPEED_1080P, CamcorderProfile.QUALITY_HIGH_SPEED_720P }; for(int h: high){ if(CamcorderProfile.hasProfile(camIdInt,h)){ CamcorderProfile p = CamcorderProfile.get(camIdInt,h); if(p!=null && p.videoFrameRate> maxProfileFps) maxProfileFps = p.videoFrameRate; } } } }catch(Exception ignored){}
        if(ranges==null || ranges.length==0){ for(int fps=10; fps<=maxProfileFps; fps+=5){ if(fps<=30 || fps%30==0) framerates.add(fps); } int[] standard={24,25,30,60,90,120}; for(int r:standard){ if(r<=maxProfileFps) framerates.add(r); } } else { for(Range<Integer> r: ranges){ if(r!=null){ for(int fps=r.getLower(); fps<= r.getUpper(); fps++){ framerates.add(fps); } } } if(maxProfileFps>30){ int[] highRates={60,90,120,240}; for(int r: highRates){ if(r<=maxProfileFps) framerates.add(r); } } }
        if(framerates.isEmpty()) framerates.add(Constants.DEFAULT_VIDEO_FRAME_RATE);
        List<Integer> list = new ArrayList<>(framerates);
        if(list.size()>20){ Set<Integer> filtered = new TreeSet<>(); int[] standard={24,25,30,50,60,90,120,240}; for(int s:standard){ if(framerates.contains(s)) filtered.add(s); } for(int f: framerates){ if(f%5==0 && f<=60) filtered.add(f); else if(f>60) filtered.add(f);} if(!filtered.contains(Constants.DEFAULT_VIDEO_FRAME_RATE) && framerates.contains(Constants.DEFAULT_VIDEO_FRAME_RATE)) filtered.add(Constants.DEFAULT_VIDEO_FRAME_RATE); list = new ArrayList<>(filtered); }
        return list;
    }
    ...
    private List<Integer> getCompatiblesVideoFrameRates(CameraType cameraType){
        ...
        if(profile==null){ List<Integer> single = new ArrayList<>(); single.add(Constants.DEFAULT_VIDEO_FRAME_RATE); return single; }
        int max = profile.videoFrameRate;
        int[] options = getResources().getIntArray(R.array.video_framerate_options);
        List<Integer> list = new ArrayList<>();
        for(int o: options){ if(o<=max) list.add(o); }
        if(list.isEmpty()) list.add(max);
        return list;
    }

    private List<VideoCodec> getCompatiblesVideoCodecs(){
        if(!cachedCodecs.isEmpty()) return cachedCodecs;
        List<VideoCodec> codecs = new ArrayList<>();
        for(VideoCodec c: VideoCodec.values()){
            if(Utils.isCodecSupported(c.getMimeType())) codecs.add(c);
        }
        if(codecs.isEmpty()) codecs.add(Constants.DEFAULT_VIDEO_CODEC);
        cachedCodecs = codecs;
        return codecs;
    }

    // Bitrate helpers (mirroring legacy semantics)
    private boolean getBitrateMode(){ return prefs.sharedPreferences.getBoolean(PREF_BITRATE_MODE_CUSTOM,false); }
    private void setBitrateMode(boolean custom){ prefs.sharedPreferences.edit().putBoolean(PREF_BITRATE_MODE_CUSTOM, custom).apply(); }
    private int getBitrateCustomValue(){ return prefs.sharedPreferences.getInt(PREF_BITRATE_CUSTOM_VALUE, getDefaultBitrate()); }
    private void setBitrateCustomValue(int value){ prefs.sharedPreferences.edit().putInt(PREF_BITRATE_CUSTOM_VALUE, value).apply(); }
    private int getDefaultBitrate(){ CamcorderProfile profile = getCamcorderProfile(prefs.getCameraSelection()); if(profile!=null) return profile.videoBitRate/1000; return 16000; }
    private int getCurrentBitrate(){ return getBitrateMode()? getBitrateCustomValue(): getDefaultBitrate(); }
    private void onResolutionOrFramerateChanged(){ refreshAllValues(); }

    // Bitrate helper text mirroring thresholds used in dialog (low<3, high>100) but for current value
    private void updateBitrateHelper(int bitrateKbps, boolean custom){ /* removed */ }
    ...
    private void showZoomRatioBottomSheet(){
        ...
        for(Float r: ratios){ items.add(new com.fadcam.ui.picker.OptionItem(String.format(Locale.getDefault(), "%.1f", r), String.format(Locale.getDefault(), "%.1fx", r))); }
        final String resultKey = "picker_result_zoom_ratio";
        getParentFragmentManager().setFragmentResultListener(resultKey, this, (k,b)->{
            String sel = b.getString(com.fadcam.ui.picker.PickerBottomSheetFragment.BUNDLE_SELECTED_ID);
            if(sel!=null && !sel.equals(currentId)){
                try{ float val = Float.parseFloat(sel); if(Math.abs(val-current)>0.001f){ prefs.setSpecificZoomRatio(cam, val); refreshAllValues(); } }catch(Exception ignored){}
            }
        });
    ...
    private List<Float> buildZoomRatioOptions(CameraType cam){
        ...
        for(float z=0.5f; z<=max+0.001f; z+=0.5f){ list.add(((float)Math.round(z*10))/10f); }
    ...
    private float getHardwareSupportedMaxZoomRatio(CameraType cam){
        ...
        Context ctx = getContext(); if(ctx==null) return defaultMaxZoom;
        try{
            CameraManager manager = (CameraManager) ctx.getSystemService(Context.CAMERA_SERVICE);
            String id = getActualCameraIdForType(cam); if(id==null) return defaultMaxZoom;
            CameraCharacteristics ch = manager.getCameraCharacteristics(id);
            if(Build.VERSION.SDK_INT>=30){
                Range<Float> range = ch.get(CameraCharacteristics.CONTROL_ZOOM_RATIO_RANGE);
                if(range!=null){ return range.getUpper(); }
            }
        }catch(Exception e){ Log.w(TAG, "Zoom ratio query failed", e); }
        return defaultMaxZoom;
    }

    // ---- Video Splitting Migration ----
    private void toggleVideoSplitting(){ boolean enabled = prefs.isVideoSplittingEnabled(); prefs.setVideoSplittingEnabled(!enabled); }

    private void showVideoSplittingBottomSheet(){
        // -------------- Fix Start for this method(showVideoSplittingBottomSheet)-----------
        boolean enabled = prefs.isVideoSplittingEnabled();
        int mb = prefs.getVideoSplitSizeMb();
        String sizeLabel;
        if(mb==500) sizeLabel = "500 MB"; else if(mb==1024) sizeLabel="1 GB"; else if(mb==2048) sizeLabel="2 GB"; else if(mb==4096) sizeLabel="4 GB"; else sizeLabel="Custom ("+mb+" MB)";
        java.util.ArrayList<com.fadcam.ui.picker.OptionItem> items = new java.util.ArrayList<>();
    items.add(new com.fadcam.ui.picker.OptionItem("size", "Change Split Size (Current: "+sizeLabel+")"));
        ...
        getParentFragmentManager().setFragmentResultListener(resultKey, this, (k,b)->{
            if(b.containsKey(com.fadcam.ui.picker.PickerBottomSheetFragment.BUNDLE_SWITCH_STATE)){
                ...
                prefs.setVideoSplittingEnabled(state); refreshAllValues();
            }
            String sel = b.getString(com.fadcam.ui.picker.PickerBottomSheetFragment.BUNDLE_SELECTED_ID);
            if(sel==null) return;
            if("size".equals(sel)){
    ...
    java.util.ArrayList<String> dep = new java.util.ArrayList<>(); dep.add("size");
    com.fadcam.ui.picker.PickerBottomSheetFragment sheet = com.fadcam.ui.picker.PickerBottomSheetFragment.newInstanceWithSwitchDependencies(
        ...
        // -------------- Fix Ended for this method(showVideoSplittingBottomSheet)-----------
    }
    ...
    private void showVideoSplitSizeBottomSheet(){
        // -------------- Fix Start for this method(showVideoSplitSizeBottomSheet)-----------
        if(!prefs.isVideoSplittingEnabled()) return;
        final int[] presetMb = {500,1024,2048,4096,-1};
        java.util.ArrayList<com.fadcam.ui.picker.OptionItem> items = new java.util.ArrayList<>();
        for(int mb: presetMb){
            if(mb==-1) items.add(new com.fadcam.ui.picker.OptionItem("custom", "Custom..."));
            else items.add(new com.fadcam.ui.picker.OptionItem(String.valueOf(mb), (mb==1024?"1 GB": mb==2048?"2 GB": mb==4096?"4 GB": mb+" MB")));
        }
        int current = prefs.getVideoSplitSizeMb();
        String currentId = null; for(int mb: presetMb){ if(mb==current){ currentId = mb==-1?null:String.valueOf(mb); break; } }
        ...
        getParentFragmentManager().setFragmentResultListener(resultKey, this, (k,b)->{
            ...
            if("custom".equals(sel)){
                ...
                try{ int mbVal = Integer.parseInt(sel); prefs.setVideoSplitSizeMb(mbVal); refreshAllValues(); }catch(Exception ignored){}
            }
        });
    String helper = getString(R.string.video_splitting_description);
    com.fadcam.ui.picker.PickerBottomSheetFragment sheet = com.fadcam.ui.picker.PickerBottomSheetFragment.newInstance(
        ...
        // -------------- Fix Ended for this method(showVideoSplitSizeBottomSheet)-----------
    }

    private void showCustomSplitSizeBottomSheet(){
        // -------------- Fix Start for this method(showCustomSplitSizeBottomSheet)-----------
        int current = prefs.getVideoSplitSizeMb();
        if(current==500 || current==1024 || current==2048 || current==4096) current=2048; // default for custom
        ...
        getParentFragmentManager().setFragmentResultListener(resultKey, this, (k,b)->{
            if(b.containsKey(com.fadcam.ui.picker.NumberInputBottomSheetFragment.RESULT_NUMBER)){
                ...
                prefs.setVideoSplitSizeMb(mb); refreshAllValues();
            }
        });
        com.fadcam.ui.picker.NumberInputBottomSheetFragment sheet = com.fadcam.ui.picker.NumberInputBottomSheetFragment.newInstance(
        ...
        // -------------- Fix Ended for this method(showCustomSplitSizeBottomSheet)-----------
    }

    private CamcorderProfile getCamcorderProfile(CameraType type){
        try{ String id = getActualCameraIdForType(type); if(id==null) return null; int camId = Integer.parseInt(id); // choose quality based on current resolution
            Size res = prefs.getCameraResolution();
            int quality = CamcorderProfile.QUALITY_HIGH;
            if(res.getWidth()>=3840 || res.getHeight()>=2160) quality = CamcorderProfile.QUALITY_2160P;
            else if(res.getWidth()>=1920 || res.getHeight()>=1080) quality = CamcorderProfile.QUALITY_1080P;
            else if(res.getWidth()>=1280 || res.getHeight()>=720) quality = CamcorderProfile.QUALITY_720P;
            if(CamcorderProfile.hasProfile(camId, quality)) return CamcorderProfile.get(camId, quality); }catch(Exception ignored){}
        return null; }

    // Simple holder mirroring legacy CameraIdInfo
    private static class CameraIdInfo{ final String id; final String displayName; CameraIdInfo(String id,String display){ this.id=id; this.displayName=display; } @Override public String toString(){ return displayName; } }
