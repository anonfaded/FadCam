...
/** Free FadDrive sync over standard WebDAV. Passwords are never persisted. */
public final class FadDriveSyncManager {
    ...
    public static final String REMOTE_FOLDER = "FadCam/";
        ...
        EXECUTOR.execute(() -> {
            try {
                ...
                try (Response response = CLIENT.newCall(builder.build()).execute()) {
                    ...
                    if (code >= 200 && code < 400) callback.onComplete("FadDrive connection OK (HTTP " + code + ").");
                    else if (code == 401 || code == 403) callback.onComplete("FadDrive authentication failed (HTTP " + code + ").");
                    else callback.onComplete("FadDrive connection failed (HTTP " + code + ").");
                }
            } catch (Exception e) {
            ...
            try {
                ...
                for (VideoItem item : videos) {
                    try {
                        ...
                        completed++;
                        callback.onProgress("Synced " + completed + "/" + videos.size() + ": " + item.name, completed, videos.size());
                    } catch (Exception e) {
                        callback.onProgress("Skipped " + item.name + ": " + safeMessage(e), completed, videos.size());
                    }
                }
    ...
    /** Builds a deterministic WebDAV destination while preserving the endpoint's encoded base path. */
    static String buildRemoteUrl(String endpoint, String fileName) throws URISyntaxException {
        URI base = validateEndpoint(endpoint);
        String safeName = URLEncoder.encode(fileName == null ? "video.mp4" : fileName, StandardCharsets.UTF_8)
                .replace("+", "%20");
        String basePath = base.getRawPath();
        if (basePath == null || basePath.isEmpty()) basePath = "/";
        if (!basePath.endsWith("/")) basePath += "/";

        // safeName and basePath are already raw/percent-encoded path components.
        // Do not pass them through a URI constructor that escapes '%' again.
        String remotePath = basePath + REMOTE_FOLDER + safeName;
        return base.getScheme() + "://" + base.getRawAuthority() + remotePath;
    }

    /** Rejects malformed endpoints and credentials embedded in URLs. */
    static URI validateEndpoint(String endpoint) throws URISyntaxException {
        String value = endpoint == null ? "" : endpoint.trim();
        if (value.isEmpty()) throw new IllegalArgumentException("WebDAV URL is required");
        URI uri = new URI(value);
        String scheme = uri.getScheme();
        if (!("https".equalsIgnoreCase(scheme) || "http".equalsIgnoreCase(scheme))) throw new IllegalArgumentException("WebDAV URL must use http:// or https://");
        if (uri.getHost() == null) throw new IllegalArgumentException("WebDAV URL has no host");
        if (uri.getUserInfo() != null) throw new IllegalArgumentException("Put credentials in the username/password fields, not the URL");
        if (uri.getQuery() != null || uri.getFragment() != null) throw new IllegalArgumentException("WebDAV URL must not contain a query or fragment");
        return uri;
    }
    ...
    private static void ensureFolder(URI base, String username, String password) throws Exception {
        ...
        String folderUrl = base.getScheme() + "://" + base.getRawAuthority() + path + REMOTE_FOLDER;
        Request.Builder builder = new Request.Builder().url(folderUrl).method("MKCOL", null);
        applyAuth(builder, username, password);
        try (Response response = CLIENT.newCall(builder.build()).execute()) {
            int code = response.code();
            if (!((code >= 200 && code < 300) || code == 301 || code == 302 || code == 405 || code == 409)) throw new IOException("Cannot create FadCam folder (HTTP " + code + ")");
        }
    }

    private static void upload(ContentResolver resolver, URI base, String username, String password, VideoItem item) throws Exception {
        if (item.size < 0 || item.size > MAX_UPLOAD_BYTES) throw new IOException("unsupported file size (maximum 4 GiB)");
    ...
    private static void applyAuth(Request.Builder builder, String username, String password) {
        if (username != null && !username.trim().isEmpty()) builder.header("Authorization", Credentials.basic(username, password == null ? "" : password));
    }

    private static List<VideoItem> queryVideos(ContentResolver resolver) {
        List<VideoItem> result = new ArrayList<>();
        String[] projection = {MediaStore.Video.Media._ID, MediaStore.Video.Media.DISPLAY_NAME, MediaStore.Video.Media.MIME_TYPE, MediaStore.Video.Media.SIZE, MediaStore.Video.Media.DATE_ADDED};
        String sort = MediaStore.Video.Media.DATE_ADDED + " DESC";
        try (Cursor cursor = resolver.query(MediaStore.Video.Media.EXTERNAL_CONTENT_URI, projection, null, null, sort)) {
    ...
    private static final class VideoItem {
        final Uri uri; final String name; final String mimeType; final long size;
        VideoItem(Uri uri, String name, String mimeType, long size) { this.uri = uri; this.name = name; this.mimeType = mimeType; this.size = size; }
    }

    private static final class ContentResolverRequestBody extends RequestBody {
        private final ContentResolver resolver; private final Uri uri; private final MediaType mediaType; private final long length;
        ContentResolverRequestBody(ContentResolver resolver, Uri uri, String mimeType, long length) {
            this.resolver = resolver; this.uri = uri; this.mediaType = MediaType.parse(mimeType == null ? "application/octet-stream" : mimeType); this.length = length;
        }
