...
public class VideoPlayerActivity extends AppCompatActivity {
            ...
            try {
                ...
                if (isPlaying) {
                    ...
                    if (Math.abs(pos - lastSavedPos) > 1000) { // only save if moved >1s
                        ...
                        SharedPreferencesManager spmLoc = SharedPreferencesManager
                                .getInstance(VideoPlayerActivity.this);
                        if (cur != null)
                            spmLoc.setSavedPlaybackPositionMs(cur.toString(), pos);
                        if (filename != null && !filename.isEmpty())
                            spmLoc.setSavedPlaybackPositionMsByFilename(filename, pos);
                        lastSavedPos = pos;
    ...
    private final CharSequence[] speedOptions = { "0.5x", "0.6x", "0.7x", "0.8x", "0.9x", "1x (Normal)", "1.5x", "2x",
            "3x", "4x", "6x", "8x", "10x" };
    private final float[] speedValues = { 0.5f, 0.6f, 0.7f, 0.8f, 0.9f, 1.0f, 1.5f, 2.0f, 3.0f, 4.0f, 6.0f, 8.0f,
            10.0f };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (videoUri != null) {
            ...
            float defSpd = sharedPreferencesManager.sharedPreferences.getFloat("pref_default_playback_speed",
            ...
            Log.e(TAG,
                    "Video URI is null. Intent Data: " + getIntent().getDataString() + ". Cannot initialize player.");
        ...
        if (timeBar instanceof com.google.android.exoplayer2.ui.DefaultTimeBar) {
            ...
            int played = androidx.core.content.ContextCompat.getColor(this, R.color.redPastel); // Fixed red color
            ...
            int scrubber = androidx.core.content.ContextCompat.getColor(this, R.color.redPastel); // Fixed red color for
                                                                                                  // thumb
                ...
                new android.view.GestureDetector.SimpleOnGestureListener() {
                    ...
                    public boolean onDoubleTap(android.view.MotionEvent e) {
                        try {
                            ...
                            int seekSec = spm != null ? spm.getPlayerSeekSeconds()
                ...
                case android.view.MotionEvent.ACTION_MOVE: {
                    ...
                    if (currentScale > 1.0f) {
                        ...
                        if (canPan) {
                            ...
                            android.view.View contentFrame = playerView
                                    .findViewById(com.google.android.exoplayer2.ui.R.id.exo_content_frame);
                            if (contentFrame != null) {
                                ...
                                currentTranslationX = Math.max(-maxTranslationX,
                                        Math.min(currentTranslationX, maxTranslationX));
                                currentTranslationY = Math.max(-maxTranslationY,
                                        Math.min(currentTranslationY, maxTranslationY));
                            }
    ...
    private void showQuickSpeedPickerSheet() {
        ...
        PickerBottomSheetFragment sheet = PickerBottomSheetFragment.newInstance(getString(R.string.quick_speed_title),
                items, selectedId, RK_QUICK_SPEED, getString(R.string.quick_speed_helper));
    ...
    private void showQuickOverlay(boolean show) {
        ...
        if (show) {
            ...
            quickSpeedOverlay.animate().alpha(0f).setDuration(120)
                    .withEndAction(() -> quickSpeedOverlay.setVisibility(View.GONE)).start();
        }
    }
    ...
    private void initializePlayer(Uri videoUri) {
        try {
            ...
            try {
                ...
                long savedMs = SharedPreferencesManager.getInstance(this)
                        .getSavedPlaybackPositionMsWithFilenameFallback(uriStr, filename);
                if (savedMs > 0) {
                ...
                if (controls != null) {
                    ...
                    if (rew != null)
                        rew.setOnClickListener(v -> {
                            try {
                                int s = spm != null ? spm.getPlayerSeekSeconds()
                                        : Constants.DEFAULT_PLAYER_SEEK_SECONDS;
                        ...
                        ffwd.setOnClickListener(v -> {
                            try {
                                ...
                                long target = Math.min(
                                        dur == com.google.android.exoplayer2.C.TIME_UNSET ? Long.MAX_VALUE : dur,
                                        cur + s * 1000L);
                                if (dur != com.google.android.exoplayer2.C.TIME_UNSET)
    ...
    private boolean isControllerVisibleSafe() {
        try {
            ...
            View timeBar = playerView.findViewById(com.google.android.exoplayer2.ui.R.id.exo_progress);
            if (timeBar != null)
                return timeBar.getVisibility() == View.VISIBLE;
        } catch (Exception ignored) {
        }
        return false;
    }
    ...
    private void showVideoSettingsSheet() {
        ...
        items.add(new OptionItem("row_playback_speed", getString(R.string.playback_speed_label), playbackSubtitle, null,
                null, null, null, null, "speed", null, null, null));
        // Row: Quick Speed
        float quick = spm != null ? spm.getQuickSpeed() : Constants.DEFAULT_QUICK_SPEED;
        ...
        items.add(new OptionItem("row_quick_speed", getString(R.string.quick_speed_title), quickSubtitle, null, null,
                null, null, null, "bolt", null, null, null));
        ...
        items.add(new OptionItem("row_mute_playback", getString(R.string.mute_playback_title), muteSubtitle, null, null,
                null, null, null, "volume_off", null, null, null));
        ...
        items.add(new OptionItem("row_keep_screen_on", getString(R.string.keep_screen_on_title), keepOnSubtitle, null,
                null, null, null, null, "visibility", null, null, null));
        ...
        items.add(new OptionItem("row_background_playback", getString(R.string.background_playback_title), bgSubtitle,
                null, null, null, null, null, "play_circle", null, null, null));
        ...
        if (timerSec == 0)
            timerSubtitle = getString(R.string.timer_off_short);
        else if (timerSec < 60)
            timerSubtitle = getString(R.string.timer_seconds_short, timerSec);
        else if (timerSec < 3600)
            timerSubtitle = getString(R.string.timer_minutes_short, timerSec / 60);
        else
            timerSubtitle = getString(R.string.timer_hours_short, timerSec / 3600);
        boolean bgEnabled = SharedPreferencesManager.getInstance(this).isBackgroundPlaybackEnabled();
        items.add(new OptionItem("row_background_playback_timer",
                getString(R.string.background_playback_timer_title_short), timerSubtitle, null, null, null, null, null,
                "timer", null, null, !bgEnabled));
        // Row: Seek amount (in seconds)
        int seekSecMain = spm != null ? spm.getPlayerSeekSeconds() : Constants.DEFAULT_PLAYER_SEEK_SECONDS;
        items.add(new OptionItem("row_seek_amount", getString(R.string.seek_amount_title),
                getString(R.string.seek_amount_subtitle, seekSecMain), null, null, null, null, null, "replay_10", null,
                null, null));

        // Row: Audio waveform visualization
        boolean waveformEnabled = spm != null ? spm.isAudioWaveformEnabled() : true;
        items.add(new OptionItem("row_audio_waveform", "Audio Waveform", waveformEnabled ? "Enabled" : "Disabled", null,
                null, null, null, waveformEnabled, "graphic_eq", null, null, null));

        String helper = getString(R.string.video_player_settings_helper_player);
        PickerBottomSheetFragment sheet = PickerBottomSheetFragment
                .newInstance(getString(R.string.video_player_settings_title), items, null, RK_VIDEO_SETTINGS, helper);
        // Clear any previous listener for this request key to avoid
        // duplicate/cross-sheet callbacks
        try {
            getSupportFragmentManager().clearFragmentResultListener(RK_VIDEO_SETTINGS);
        } catch (Exception ignored) {
        }
        getSupportFragmentManager().setFragmentResultListener(RK_VIDEO_SETTINGS, this, (key, bundle) -> {
            ...
            if ("row_playback_speed".equals(sel)) {
                ...
                itemsTimer.add(new OptionItem("t_off", getString(R.string.timer_off), null, null, null, null, null,
                        null, "block", null, null, null));
                itemsTimer.add(new OptionItem("t_30s", getString(R.string.timer_30_seconds), null, null, null, null,
                        null, null, "timer", null, null, null));
                itemsTimer.add(new OptionItem("t_1m", getString(R.string.timer_1_minute), null, null, null, null, null,
                        null, "timer", null, null, null));
                itemsTimer.add(new OptionItem("t_5m", getString(R.string.timer_5_minutes), null, null, null, null, null,
                        null, "timer", null, null, null));
                itemsTimer.add(new OptionItem("t_15m", getString(R.string.timer_15_minutes), null, null, null, null,
                        null, null, "timer", null, null, null));
                itemsTimer.add(new OptionItem("t_30m", getString(R.string.timer_30_minutes), null, null, null, null,
                        null, null, "timer", null, null, null));
                itemsTimer.add(new OptionItem("t_1h", getString(R.string.timer_1_hour), null, null, null, null, null,
                        null, "timer", null, null, null));
                itemsTimer.add(new OptionItem("t_custom", getString(R.string.timer_custom_label), null, null, null,
                        null, null, null, "edit", null, null, null));
                ...
                if (cur == 30)
                    ...
                    selId = "t_custom"; // show Custom checked for non-preset values
                PickerBottomSheetFragment sheetTimer = PickerBottomSheetFragment.newInstance(
                        getString(R.string.background_playback_timer_title), itemsTimer, selId, RK,
                ...
                getSupportFragmentManager().setFragmentResultListener(RK, this, (rkRes, resBundle) -> {
                    ...
                    if ("t_custom".equals(sel2)) {
                        ...
                        com.fadcam.ui.picker.NumberInputBottomSheetFragment num = com.fadcam.ui.picker.NumberInputBottomSheetFragment
                                .newInstance(
                                        getString(R.string.timer_custom_title), 1, 86400, 60,
                                        getString(R.string.timer_custom_placeholder), 5, 3600,
                                        getString(R.string.timer_custom_low_hint),
                                        getString(R.string.timer_custom_high_hint), RK_NUM);
                        android.os.Bundle _b = num.getArguments() != null ? num.getArguments()
                                : new android.os.Bundle();
                        _b.putString(com.fadcam.ui.picker.NumberInputBottomSheetFragment.ARG_DESCRIPTION,
                        ...
                        android.os.Bundle _nb = num.getArguments() != null ? num.getArguments()
                                : new android.os.Bundle();
                        _nb.putString(com.fadcam.ui.picker.NumberInputBottomSheetFragment.ARG_DESCRIPTION,
                        ...
                        getSupportFragmentManager().setFragmentResultListener(RK_NUM, this, (rkN, nb) -> {
                            int minutes = nb.getInt(com.fadcam.ui.picker.NumberInputBottomSheetFragment.RESULT_NUMBER,
                                    0);
                            if (minutes > 0) {
                                ...
                                if (root != null) {
                                    ...
                                    if (sub != null) {
                                        sub.setText(minutes < 60 ? getString(R.string.timer_minutes_short, minutes)
                                                : getString(R.string.timer_hours_short, minutes / 60));
                                    }
                                }
                            }
                        });
                        ...
                        // avoid view overlap
                        try {
                            if (sheetTimer != null && sheetTimer.isAdded())
                    ...
                    if (root != null) {
                        ...
                        if (sub != null) {
                            if (seconds == 0)
                                sub.setText(getString(R.string.timer_off_short));
                            else if (seconds < 60)
                                sub.setText(getString(R.string.timer_seconds_short, seconds));
                            else if (seconds < 3600)
                                sub.setText(getString(R.string.timer_minutes_short, seconds / 60));
                            else
                                sub.setText(getString(R.string.timer_hours_short, seconds / 3600));
                        }
                    }
                });
                ...
                itemsSeek.add(
                        new OptionItem("s_5", "5s", null, null, null, null, null, null, "replay_10", null, null, null));
                itemsSeek.add(new OptionItem("s_10", "10s", null, null, null, null, null, null, "replay_10", null, null,
                        null));
                itemsSeek.add(new OptionItem("s_15", "15s", null, null, null, null, null, null, "replay_10", null, null,
                        null));
                itemsSeek.add(new OptionItem("s_30", "30s", null, null, null, null, null, null, "replay_10", null, null,
                        null));
                itemsSeek.add(new OptionItem("s_custom", getString(R.string.seek_amount_custom), null, null, null, null,
                        null, null, "edit", null, null, null));
                int curS = SharedPreferencesManager.getInstance(this).getPlayerSeekSeconds();
                String selId = "s_" + curS;
                if (curS != 5 && curS != 10 && curS != 15 && curS != 30)
                    selId = "s_custom";
                PickerBottomSheetFragment sheetSeek = PickerBottomSheetFragment.newInstance(
                        getString(R.string.seek_amount_title), itemsSeek, selId, RK,
                        getString(R.string.seek_amount_helper));
                try {
                    getSupportFragmentManager().clearFragmentResultListener(RK);
                } catch (Exception ignored) {
                }
                getSupportFragmentManager().setFragmentResultListener(RK, this, (rkRes, resBundle) -> {
                    ...
                    if ("s_custom".equals(s)) {
                        ...
                        com.fadcam.ui.picker.NumberInputBottomSheetFragment num = com.fadcam.ui.picker.NumberInputBottomSheetFragment
                                .newInstance(
                                        getString(R.string.seek_amount_custom_title), 1, 300, 10,
                                        getString(R.string.universal_enter_number), 5, 60,
                                        getString(R.string.seek_amount_custom_low_hint),
                                        getString(R.string.seek_amount_custom_high_hint), RK_NUM);
                        try {
                            getSupportFragmentManager().clearFragmentResultListener(RK_NUM);
                        } catch (Exception ignored) {
                        }
                        getSupportFragmentManager().setFragmentResultListener(RK_NUM, this, (rkN, nb) -> {
                            int val = nb.getInt(com.fadcam.ui.picker.NumberInputBottomSheetFragment.RESULT_NUMBER, 0);
                            if (val > 0) {
                                SharedPreferencesManager.getInstance(this).setPlayerSeekSeconds(val);
                                View root = findViewById(android.R.id.content);
                                if (root != null) {
                                    TextView sub = root.findViewById(R.id.sub_seek_amount);
                                    if (sub != null)
                                        sub.setText(getString(R.string.seek_amount_subtitle, val));
                                }
                            }
                        });
                        ...
                        new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
                            try {
                                num.show(getSupportFragmentManager(), "video_seek_amount_custom_sheet");
                            } catch (Exception ignored) {
                            }
                        }, 180);
                        return;
                    ...
                    if (s.startsWith("s_")) {
                        try {
                            ...
                            SharedPreferencesManager.getInstance(this).setPlayerSeekSeconds(v);
                            View root = findViewById(android.R.id.content);
                            if (root != null) {
                                TextView sub = root.findViewById(R.id.sub_seek_amount);
                                if (sub != null)
                                    sub.setText(getString(R.string.seek_amount_subtitle, v));
                            }
                        } catch (NumberFormatException ignored) {
                        }
                    }
    ...
    private void showPlaybackSpeedPickerSheet() {
        ...
        PickerBottomSheetFragment sheet = PickerBottomSheetFragment.newInstance(
                getString(R.string.playback_speed_title), items, selectedId, RK_PLAYBACK_SPEED,
    ...
    private void showMuteSwitchSheet() {
        ...
        PickerBottomSheetFragment sheet = PickerBottomSheetFragment.newInstanceWithSwitch(
                getString(R.string.mute_playback_title), new ArrayList<>(), null, RK, helper,
                getString(R.string.mute_playback_title), enabled);
            ...
            if (android.os.Build.VERSION.SDK_INT >= 33) {
                if (androidx.core.content.ContextCompat.checkSelfPermission(this,
                    ...
                    androidx.core.app.ActivityCompat.requestPermissions(this,
                            new String[] { android.Manifest.permission.POST_NOTIFICATIONS }, 1010);
                }
            }
        } catch (Exception ignored) {
        }
        ...
        try {
            ...
            if (overlay == null) {
                ...
                lp.gravity = android.view.Gravity.CENTER_VERTICAL
                        | (forward ? android.view.Gravity.END : android.view.Gravity.START);
            ...
            try {
                ...
                if (p instanceof android.widget.FrameLayout) {
                    android.widget.FrameLayout.LayoutParams lp = (android.widget.FrameLayout.LayoutParams) overlayView
                            .getLayoutParams();
            ...
            overlayView
                    .animate().alpha(1f).setDuration(
                            80)
                    .withEndAction(() -> overlayView.postDelayed(() -> overlayView.animate().alpha(0f).setDuration(250)
                            .withEndAction(() -> overlayView.setVisibility(View.GONE)), 500))
                    .start();
        } catch (Exception ignored) {
        }
    }
            ...
            if (bg && isPlaying && hasPostNotificationsPermission()) {
                ...
                try {
                    android.content.Intent svc = new android.content.Intent(this,
                            ...
                            .putExtra(com.fadcam.services.PlaybackService.EXTRA_SPEED,
                                    player.getPlaybackParameters().speed)
                            .putExtra(com.fadcam.services.PlaybackService.EXTRA_MUTED, player.getVolume() == 0f)
                            .putExtra(com.fadcam.services.PlaybackService.EXTRA_POSITION_MS,
                ...
                try {
                    android.content.Intent stop = new android.content.Intent(this,
                            com.fadcam.services.PlaybackService.class)
        ...
        if (playerView != null) {
            try {
                ...
                if (contentFrame != null) {
                    ...
                    android.view.View shutter = playerView
                            .findViewById(com.google.android.exoplayer2.ui.R.id.exo_shutter);
                    if (shutter != null) {
    ...
    private void updateResetZoomButtonVisibility() {
        if (resetZoomButton != null) {
            if (Math.abs(currentScale - 1.0f) > 0.1f || Math.abs(currentTranslationX) > 10f
