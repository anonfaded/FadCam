...
public class ProductionToolsFragment extends Fragment implements SensorEventListener {
    ...
    private void buildThermal(LinearLayout root) {
        ...
        arm.setOnClickListener(v -> { thermalGuardArmed=!thermalGuardArmed; arm.setText(thermalGuardArmed?"Thermal Guard ON":"Enable Thermal Guard"); Toast.makeText(requireContext(),thermalGuardArmed?"Thermal protection armed":"Thermal protection disabled",Toast.LENGTH_SHORT).show(); });
    ...
    private void buildAudioVision(LinearLayout root) {
        ...
        TextView note=text("When armed, a sustained sound above the threshold starts the FadCam recording service. Stop it from the main recording controls.",13,0xFFBDBDBD); root.addView(note,lp(-1,-2));
    }

    private void buildSoundMeter(LinearLayout root, boolean vision) {
        liveValue=metric(root,"Sound level: — dB");
        TextView status=text("Microphone level meter",14,0xFFBDBDBD); root.addView(status,lp(-1,-2));
    ...
    private void startAudio(){ if(audioRunning)return; audioRunning=true; final int sampleCount=Math.max(1024,AudioRecord.getMinBufferSize(16000,AudioFormat.CHANNEL_IN_MONO,AudioFormat.ENCODING_PCM_16BIT)); final int buffer=sampleCount*2; audioExecutor=Executors.newSingleThreadExecutor(); audioExecutor.execute(()->{try{audioRecord=new AudioRecord(MediaRecorder.AudioSource.MIC,16000,AudioFormat.CHANNEL_IN_MONO,AudioFormat.ENCODING_PCM_16BIT,buffer);audioRecord.startRecording();short[] data=new short[sampleCount];long loudSince=0;while(audioRunning){int n=audioRecord.read(data,0,data.length);if(n<=0)continue;double sum=0;for(int i=0;i<n;i++)sum+=(double)data[i]*data[i];double rms=Math.sqrt(sum/n);float db=(float)(20*Math.log10(Math.max(rms,1)/32768.0)+90);ui.post(()->{if(liveValue!=null)liveValue.setText(String.format(Locale.US,"Sound level: %.1f dB",db));});if(audioArmed&&db>=noiseThreshold){if(loudSince==0)loudSince=System.currentTimeMillis();if(System.currentTimeMillis()-loudSince>=700){startRecordingService();audioArmed=false;}}else loudSince=0;}}catch(Exception e){ui.post(()->Toast.makeText(requireContext(),"Microphone unavailable: "+e.getMessage(),Toast.LENGTH_LONG).show());}finally{if(audioRecord!=null){try{audioRecord.stop();}catch(Exception ignored){}audioRecord.release();audioRecord=null;}}});}

    private void buildSchedule(LinearLayout root){
        TextView help=text("Create a one-time recording alarm. The receiver starts the recording service and automatically stops it after the selected duration.",14,0xFFCCCCCC);root.addView(help,lp(-1,-2));
        ...
        schedule.setOnClickListener(v->{try{SimpleDateFormat fmt=new SimpleDateFormat("yyyy-MM-dd HH:mm",Locale.US);long when=fmt.parse(date.getText().toString().trim()).getTime();if(when<=System.currentTimeMillis())throw new IllegalArgumentException("Choose a future time");String value=mins.getText().toString().trim();long duration=1000L*60L*Math.max(1,Long.parseLong(value.isEmpty()?"30":value));ProductionScheduleReceiver.schedule(requireContext(),when,duration);pending.setText("Scheduled for "+date.getText());Toast.makeText(requireContext(),"Recording scheduled",Toast.LENGTH_SHORT).show();}catch(Exception e){Toast.makeText(requireContext(),"Invalid schedule: "+e.getMessage(),Toast.LENGTH_LONG).show();}});
    }
    ...
    private void buildProfiles(LinearLayout root){
        ...
        refresh[0] = () -> { list.removeAllViews(); for(Map.Entry<String,?> e:p.getAll().entrySet()){ if(!e.getKey().startsWith("production_profile_")) continue; String[] vals=String.valueOf(e.getValue()).split("\\|",-1); LinearLayout row=new LinearLayout(requireContext()); row.setGravity(Gravity.CENTER_VERTICAL); TextView t=text(e.getKey().substring("production_profile_".length())+" • "+(vals.length>0?vals[0]:""),14,0xFFFFFFFF); row.addView(t,new LinearLayout.LayoutParams(0,-2,1)); Button load=button("Load"); row.addView(load,new LinearLayout.LayoutParams(dp(90),-2)); Button del=button("Delete"); row.addView(del,new LinearLayout.LayoutParams(dp(90),-2)); load.setOnClickListener(v->{if(vals.length>0){int q=indexOf(quality,vals[0]);if(q>=0)quality.setSelection(q);}if(vals.length>1){int c=indexOf(camera,vals[1]);if(c>=0)camera.setSelection(c);}name.setText(e.getKey().substring("production_profile_".length()));}); del.setOnClickListener(v->{p.edit().remove(e.getKey()).apply();refresh[0].run();}); list.addView(row); } };
        save.setOnClickListener(v->{String n=name.getText().toString().trim();if(n.isEmpty()){Toast.makeText(requireContext(),"Enter a profile name",Toast.LENGTH_SHORT).show();return;}p.edit().putString("production_profile_"+n,quality.getSelectedItem().toString()+"|"+camera.getSelectedItem().toString()).apply();refresh[0].run();Toast.makeText(requireContext(),"Profile saved",Toast.LENGTH_SHORT).show();});refresh[0].run();
    }
    ...
    private void buildSensors(LinearLayout root){
        ...
        TextView note=text("This dashboard reads available hardware in real time. Missing sensors are reported instead of faking values.",14,0xFFCCCCCC);root.addView(note,lp(-1,-2));
    ...
    private void buildParking(LinearLayout root){liveValue=metric(root,"No parking location saved");requestLocation();Button save=button("Save Current Parking Spot");root.addView(save,lp(-1,-2));Button navigate=button("Navigate to Saved Spot");root.addView(navigate,lp(-1,-2));save.setOnClickListener(v->{if(lastLocation==null){Toast.makeText(requireContext(),"Waiting for GPS fix",Toast.LENGTH_SHORT).show();return;}prefs().edit().putString("parking_lat",Double.toString(lastLocation.getLatitude())).putString("parking_lon",Double.toString(lastLocation.getLongitude())).apply();liveValue.setText(String.format(Locale.US,"Saved: %.6f, %.6f",lastLocation.getLatitude(),lastLocation.getLongitude()));});navigate.setOnClickListener(v->{String la=prefs().getString("parking_lat",null),lo=prefs().getString("parking_lon",null);if(la==null||lo==null){Toast.makeText(requireContext(),"No saved parking spot",Toast.LENGTH_SHORT).show();return;}Intent i=new Intent(Intent.ACTION_VIEW,android.net.Uri.parse("geo:"+la+","+lo+"?q="+la+","+lo+"(Parking)"));startActivity(i);});}

    private void buildQr(LinearLayout root){EditText input=edit("Text, URL, contact or production link");root.addView(input,lp(-1,-2));Button gen=button("Generate QR Code");root.addView(gen,lp(-1,-2));ImageView image=new ImageView(requireContext());image.setAdjustViewBounds(true);root.addView(image,lp(-1,dp(280)));gen.setOnClickListener(v->{String value=input.getText().toString().trim();if(value.isEmpty()){Toast.makeText(requireContext(),"Enter content first",Toast.LENGTH_SHORT).show();return;}try{BitMatrix m=new MultiFormatWriter().encode(value,BarcodeFormat.QR_CODE,700,700);Bitmap b=Bitmap.createBitmap(700,700,Bitmap.Config.ARGB_8888);for(int x=0;x<700;x++)for(int y=0;y<700;y++)b.setPixel(x,y,m.get(x,y)?Color.BLACK:Color.WHITE);image.setImageBitmap(b);}catch(Exception e){Toast.makeText(requireContext(),"QR generation failed",Toast.LENGTH_SHORT).show();}});}
    ...
    private void startRecordingService(){try{Intent i=new Intent(requireContext(),RecordingService.class);i.setAction(Constants.INTENT_ACTION_START_RECORDING);ServiceStartPolicy.startRecordingAction(requireContext(),i);Toast.makeText(requireContext(),"FadCam recording started",Toast.LENGTH_SHORT).show();}catch(Exception e){Toast.makeText(requireContext(),"Could not start recording: "+e.getMessage(),Toast.LENGTH_LONG).show();}}
