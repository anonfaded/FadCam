...
public final class ProductionControlState {
    ...
    private static final int MIN_DURATION_MS = 100, MAX_DURATION_MS = 3000, MIN_CAMERA_SLOT = 1, MAX_CAMERA_SLOT = 6;
    ...
    public ProductionControlState(ProductionScene previewScene, ProductionScene programScene, Transition transition, int transitionDurationMs) {
    ...
    public ProductionControlState(ProductionScene previewScene, ProductionScene programScene, int previewCameraSlot, int programCameraSlot, Transition transition, int transitionDurationMs) {
        ...
        this.previewCameraSlot = clampCameraSlot(previewCameraSlot); this.programCameraSlot = clampCameraSlot(programCameraSlot);
        this.transition = transition == null ? Transition.CUT : transition; this.transitionDurationMs = clampDuration(transitionDurationMs);
    }
    public static ProductionControlState load(Context context) {
        SharedPreferences p = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE);
        return new ProductionControlState(readScene(p, KEY_PREVIEW, ProductionScene.VIDEO), readScene(p, KEY_PROGRAM, ProductionScene.VIDEO),
                p.getInt(KEY_PREVIEW_CAMERA, 1), p.getInt(KEY_PROGRAM_CAMERA, 1), readTransition(p.getString(KEY_TRANSITION, Transition.CUT.name())), p.getInt(KEY_DURATION_MS, 500));
    }
    /** Saves state; a broadcast is emitted only when PROGRAM actually changes. */
    public void save(Context context) {
        SharedPreferences p = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE);
        String oldProgram = p.getString(KEY_PROGRAM, null);
        p.edit().putString(KEY_PREVIEW, previewScene.name()).putString(KEY_PROGRAM, programScene.name())
                .putInt(KEY_PREVIEW_CAMERA, previewCameraSlot).putInt(KEY_PROGRAM_CAMERA, programCameraSlot)
                .putString(KEY_TRANSITION, transition.name()).putInt(KEY_DURATION_MS, transitionDurationMs).apply();
        if (oldProgram != null && !oldProgram.equals(programScene.name())) notifyProgramPublished(context);
    }
    public void publishProgram(Context context) { save(context); notifyProgramPublished(context); }
    private static void notifyProgramPublished(Context context) {
        Intent intent = new Intent(ACTION_PROGRAM_PUBLISHED); intent.setPackage(context.getPackageName()); context.sendBroadcast(intent);
    }
    ...
    public ProductionScene getPreviewScene() { return previewScene; } public ProductionScene getProgramScene() { return programScene; }
    public int getPreviewCameraSlot() { return previewCameraSlot; } public int getProgramCameraSlot() { return programCameraSlot; }
    public Transition getTransition() { return transition; } public int getTransitionDurationMs() { return transitionDurationMs; }
