...
import com.servalabs.cam.ui.faditor.EditorMiniInfoBottomSheet;
...
/**
 * Entry-point fragment for the EditorMini Mini tab.
 *
 ...
 * cards. Launches {@link EditorMiniEditorActivity} for full-screen editing
 * via a video source bottom sheet.</p>
 */
public class EditorMiniMiniFragment extends BaseFragment {

    private static final String TAG = "EditorMiniMiniFragment";
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        FLog.d("EditorMiniMiniFragment", "✅ onCreate called!");
        ...
        if (exportProgressReceiver != null && !isExportReceiverRegistered) {
            try {
                FLog.d("EditorMiniMiniFragment", "Registering receiver in onCreate");
                ...
                FLog.d("EditorMiniMiniFragment", "✅ Export receiver registered in onCreate");
            } catch (Exception e) {
                FLog.e("EditorMiniMiniFragment", "Error registering export receiver in onCreate: " + e.getMessage(), e);
            }
        }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        FLog.d("EditorMiniMiniFragment", "✅ onCreateView called!");
        View view = inflater.inflate(R.layout.fragment_editor_mini_mini, container, false);
        ...
        // Info icon → opens EditorMini info bottom sheet
        View btnInfo = view.findViewById(R.id.btn_info);
        if (btnInfo != null) {
            btnInfo.setOnClickListener(v -> {
                EditorMiniInfoBottomSheet.newInstance()
                        .show(getChildFragmentManager(), "editor_mini_info");
            });
        }
        ...
        exportDock = view.findViewById(R.id.editor_mini_export_dock);
        exportDockIcon = view.findViewById(R.id.editor_mini_export_dock_icon);
        exportDockTitle = view.findViewById(R.id.editor_mini_export_dock_title);
        exportDockProgress = view.findViewById(R.id.editor_mini_export_dock_progress);
        exportDockCurrent = view.findViewById(R.id.editor_mini_export_dock_current);
        exportDockEta = view.findViewById(R.id.editor_mini_export_dock_eta);
        exportDockSummary = view.findViewById(R.id.editor_mini_export_dock_summary);
        exportDockOk = view.findViewById(R.id.editor_mini_export_dock_ok);
    ...
    private void initializeExportProgressReceiver() {
        ...
        FLog.d("EditorMiniMiniFragment", "Initializing export progress broadcast receiver...");

        exportProgressReceiver = new android.content.BroadcastReceiver() {
            @Override
            public void onReceive(android.content.Context context, android.content.Intent intent) {
                if (!isAdded()) {
                    FLog.d("EditorMiniMiniFragment", "Received broadcast but fragment not added, ignoring");
                    return; // Fragment detached
                }
                
                String action = intent.getAction();
                if (action == null) {
                    FLog.d("EditorMiniMiniFragment", "Received broadcast with null action");
                    return;
                }
                
                FLog.d("EditorMiniMiniFragment", "Export broadcast received: action=" + action);
                ...
                if (com.servalabs.cam.ui.faditor.export.ExportService.ACTION_EXPORT_STARTED.equals(action)) {
                    ...
                    FLog.d("EditorMiniMiniFragment", "Export started: " + outputPath);
                    // Post to main thread
                    ...
                    FLog.d("EditorMiniMiniFragment", "Export progress: " + progress);
                    // Post to main thread
                    new android.os.Handler(android.os.Looper.getMainLooper()).post(() -> 
                    ...
                    String outputPath = intent.getStringExtra(
                        com.servalabs.cam.ui.faditor.export.ExportService.EXTRA_OUTPUT_PATH);
                    FLog.d("EditorMiniMiniFragment", "Export completed: " + outputPath);
                    // Post to main thread
                    new android.os.Handler(android.os.Looper.getMainLooper()).post(() -> 
                    ...
                    FLog.d("EditorMiniMiniFragment", "Export error: " + errorMsg);
                    // Post to main thread
                    new android.os.Handler(android.os.Looper.getMainLooper()).post(() -> 
                    ...
                    FLog.d("EditorMiniMiniFragment", "Export cancelled");
                    // Post to main thread
                    new android.os.Handler(android.os.Looper.getMainLooper()).post(() -> 
                        onExportCancelled());
                }
        ...
        FLog.d("EditorMiniMiniFragment", "Broadcast receiver initialized successfully");
    }

    @Override
    public void onResume() {
        super.onResume();
        FLog.d("EditorMiniMiniFragment", "onResume called");
        refreshRecentProjects();
    }
    ...
    public void onPause() {
        super.onPause();
        FLog.d("EditorMiniMiniFragment", "onPause called");
    }
    ...
    public void onDestroy() {
        ...
        if (exportProgressReceiver != null && isExportReceiverRegistered) {
            try {
                ...
                FLog.w("EditorMiniMiniFragment", "Error unregistering export receiver: " + e.getMessage());
            }
    ...
    private void updateSelectionUI() {
        ...
        if (tvSelectionCount != null) {
            if (count == 0) {
                tvSelectionCount.setText(R.string.editor_mini_select_items);
            } else {
                tvSelectionCount.setText(getString(R.string.editor_mini_selected_count, count));
            }
        }
        if (btnDeleteSelected != null) {
    ...
    private void confirmDeleteSelected() {
        ...
        new MaterialAlertDialogBuilder(requireContext())
                .setTitle(R.string.editor_mini_delete_projects_title)
                .setMessage(getString(R.string.editor_mini_delete_projects_message, count))
                .setPositiveButton(R.string.editor_mini_delete_confirm, (dialog, which) -> {
                ...
                .setNegativeButton(R.string.editor_mini_cancel, null)
                .show();
    }
    ...
    private void openVideoPicker() {
        ...
        try {
            ...
            Toast.makeText(requireContext(),
                    R.string.editor_mini_error_picker, Toast.LENGTH_SHORT).show();
        }
    }
    ...
    private void launchEditor(@NonNull Uri videoUri, @Nullable String projectId) {
        Intent intent = new Intent(requireContext(), EditorMiniEditorActivity.class);
        intent.setData(videoUri);
        intent.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION);
        if (projectId != null) {
            intent.putExtra(EditorMiniEditorActivity.EXTRA_PROJECT_ID, projectId);
        }
    ...
    private String getCacheSizeText(@NonNull ProjectStorage.ProjectSummary summary) {
        try {
            ...
            EditorMiniProject project = projectStorage.load(summary.id);
    ...
    private void handleExportCompleted(String outputPath) {
        ...
        if (exportDockTitle != null) {
            exportDockTitle.animateSlot(getString(R.string.editor_mini_export_completed), 300);
        }
    ...
    private void handleExportError(String errorMessage) {
        ...
        // Update title
        if (exportDockTitle != null) {
            exportDockTitle.animateSlot(getString(R.string.editor_mini_export_failed), 300);
        }
    ...
    private void checkAndBindExportService() {
        try {
            ...
            new android.os.Handler(android.os.Looper.getMainLooper()).postDelayed(() -> {
                ...
                // For now, the EditorMini editor will notify us when it launches the service
            }, 100);
        } catch (Exception e) {
     ...
     * Should be called by the ExportService or from the EditorMini editor.
     */
    public void onExportStarted(String outputPath) {
        ...
        // Update dock content
        if (exportDockTitle != null) {
            exportDockTitle.setText(getString(R.string.editor_mini_export_progress));
        }
        if (exportDockCurrent != null) {
    ...
    public void onExportCancelled() {
        ...
        // Update title
        if (exportDockTitle != null) {
            exportDockTitle.animateSlot(getString(R.string.editor_mini_export_cancelled), 300);
        }
