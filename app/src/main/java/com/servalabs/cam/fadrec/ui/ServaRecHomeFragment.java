package com.servalabs.cam.fadrec.ui;
...
import com.servalabs.cam.fadrec.MediaProjectionHelper;
import com.servalabs.cam.fadrec.ScreenRecordingState;
import com.servalabs.cam.fadrec.services.ScreenRecordingService;
...
import com.servalabs.cam.ui.utils.AnimatedTextView;
import com.servalabs.cam.utils.ServiceUtils;
...
public class ServaRecHomeFragment extends HomeFragment {
            ...
            result -> {
                ...
                if (resultCode == Activity.RESULT_OK && data != null) {
                    if (pendingPreviewOnlyPermission) {
                        ...
                        Toast.makeText(
                            requireContext(),
                            com.servalabs.cam.R.string.fullscreen_preview_not_recording,
                            Toast.LENGTH_LONG
                        ).show();
                    } else {
        ...
        overlayPermissionLauncher = registerForActivityResult(
            ...
            result -> {
                if (android.provider.Settings.canDrawOverlays(requireContext())) {
                    ...
                    if (getView() != null) {
                        View cardFloatingControls = getView().findViewById(com.servalabs.cam.R.id.cardFloatingControls);
                        if (cardFloatingControls != null) {
                            com.servalabs.cam.ui.AvatarToggleView switchFloatingControls = 
                                cardFloatingControls.findViewById(com.servalabs.cam.R.id.switchFloatingControls);
                    ...
                    com.servalabs.cam.Utils.showQuickToast(
                        requireContext(),
                        com.servalabs.cam.R.string.floating_controls_permission_needed
                    );
                }
            }
        );
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        View rowCamera = view.findViewById(com.servalabs.cam.R.id.rowCamera);
        if (rowCamera != null) {
            rowCamera.setOnClickListener(v -> {
                if (getActivity() != null) {
                    com.servalabs.cam.ui.OverlayNavUtil.show(
                            getActivity(),
                            new com.servalabs.cam.fadrec.ui.ScreenRecordingSettingsFragment(),
    ...
    private void registerAnnotationServiceReceiver() {
        ...
        if (annotationServiceReceiver == null) {
            annotationServiceReceiver = new BroadcastReceiver() {
                ...
                public void onReceive(Context context, Intent intent) {
                    ...
                    if ("com.servalabs.cam.fadrec.ANNOTATION_SERVICE_STOPPED".equals(action) || 
                        "com.servalabs.cam.fadrec.ACTION_SERVICE_TERMINATED".equals(action)) {
                    ...
                    } else if ("com.servalabs.cam.fadrec.ANNOTATION_SERVICE_READY".equals(action)) {
                    ...
                    } else if ("com.servalabs.cam.fadrec.ANNOTATION_SERVICE_PERMISSION_ERROR".equals(action)) {
        ...
        try {
            ...
            filter.addAction("com.servalabs.cam.fadrec.ANNOTATION_SERVICE_STOPPED");
            filter.addAction("com.servalabs.cam.fadrec.ACTION_SERVICE_TERMINATED");
            filter.addAction("com.servalabs.cam.fadrec.ANNOTATION_SERVICE_READY");
            filter.addAction("com.servalabs.cam.fadrec.ANNOTATION_SERVICE_PERMISSION_ERROR");
    ...
    protected void resetUIButtonsToIdleState() {
        ...
        try {
            ...
            if (currentState == ScreenRecordingState.NONE) {
                ...
                if (buttonStartStop != null) {
                    buttonStartStop.setText(com.servalabs.cam.R.string.button_stop);
                    buttonStartStop.setIcon(
                        AppCompatResources.getDrawable(getContext(), com.servalabs.cam.R.drawable.stop_rounded)
                    );
                    buttonStartStop.setBackgroundTintList(
                        android.content.res.ColorStateList.valueOf(
                            androidx.core.content.ContextCompat.getColor(getContext(), com.servalabs.cam.R.color.button_stop)
                        )
                    );
                    buttonStartStop.setEnabled(true);
                    buttonStartStop.setAlpha(1.0f);
            ...
            if (buttonPauseResume != null) {
                ...
                // Icon-only, no text label (like ServaCam)
                if (currentState == ScreenRecordingState.PAUSED) {
                    ...
                    buttonPauseResume.setIcon(
                        AppCompatResources.getDrawable(getContext(), com.servalabs.cam.R.drawable.pause_rounded)
                    );
                }
            }
    ...
    private void hideCameraControls(View rootView) {
        ...
        View camSwitchBtn = rootView.findViewById(com.servalabs.cam.R.id.buttonCamSwitch);
        ...
        // Hide torch button
        View torchBtn = rootView.findViewById(com.servalabs.cam.R.id.buttonTorchSwitch);
        if (torchBtn != null) {
        ...
        View tileAfToggle = rootView.findViewById(com.servalabs.cam.R.id.tile_af_toggle);
        if (tileAfToggle != null) {
            tileAfToggle.setVisibility(View.GONE);
        }
        View tileExp = rootView.findViewById(com.servalabs.cam.R.id.tile_exp);
        if (tileExp != null) {
            tileExp.setVisibility(View.GONE);
        }
        
        View tileZoom = rootView.findViewById(com.servalabs.cam.R.id.tile_zoom);
        if (tileZoom != null) {
            tileZoom.setVisibility(View.GONE);
        }
        
        // Hide recording controls title (AF · Exposure · Zoom)
        View tvRecordingControlsTitle = rootView.findViewById(com.servalabs.cam.R.id.tv_recording_controls_title);
    ...
    private void setupFloatingControlsCard(View rootView) {
        ...
        View cardFloatingControls = getLayoutInflater().inflate(
            com.servalabs.cam.R.layout.card_floating_controls,
            null
        );
    ...
    private void requestOverlayPermission() {
        ...
        androidx.appcompat.app.AlertDialog permissionDialog = new com.google.android.material.dialog.MaterialAlertDialogBuilder(requireContext())
            .setTitle(com.servalabs.cam.R.string.floating_controls_permission_title)
            .setMessage(com.servalabs.cam.R.string.floating_controls_permission_message)
            .setPositiveButton(com.servalabs.cam.R.string.floating_controls_permission_grant, (dialog, which) -> {
                try {
    ...
    private void startAnnotationService() {
        try {
            ...
            if (loadingDialog == null || !loadingDialog.isShowing()) {
                ...
                android.view.View dialogView = inflater.inflate(com.servalabs.cam.R.layout.dialog_loading, null);
    ...
    private void updateCardForScreenRecording(View rootView) {
        ...
        rootView.post(() -> {
            ...
            View oldIconView = rootView.findViewById(com.servalabs.cam.R.id.ivCameraIcon);
            TextView tvCameraTitle = rootView.findViewById(com.servalabs.cam.R.id.tvCameraTitle);
            TextView tvCameraSubtitle = rootView.findViewById(com.servalabs.cam.R.id.tvCameraSubtitle);
            ...
            if (oldIconView != null && oldIconView.getParent() instanceof android.view.ViewGroup) {
                ...
                ivScreenRecordIcon.setId(com.servalabs.cam.R.id.ivCameraIcon); // Keep same ID
                ...
                ivScreenRecordIcon.setImageResource(com.servalabs.cam.R.drawable.screen_recorder);
                
                // Apply tint (green color matching camera icon)
                ivScreenRecordIcon.setColorFilter(
                    ContextCompat.getColor(requireContext(), com.servalabs.cam.R.color.greenPastel),
            ...
            if (tvCameraTitle != null) {
                ...
                com.servalabs.cam.SharedPreferencesManager prefs = com.servalabs.cam.SharedPreferencesManager.getInstance(requireContext());
    ...
    private void handleOverlayRecordingStart() {
        ...
        Intent intent = new Intent(requireContext(), com.servalabs.cam.fadrec.ui.TransparentPermissionActivity.class);
    ...
    private void renderPrimaryButtonForRecordingState(boolean recordingActive) {
        ...
        applyButtonTransition(
            ...
            () -> {
                buttonStartStop.setBackgroundTintList(
                    recordingActive
                        ? ContextCompat.getColorStateList(requireContext(), com.servalabs.cam.R.color.button_stop)
    ...
    private void updateUIForRecordingState() {
        ...
        MaterialButton buttonStartStop = rootView.findViewById(com.servalabs.cam.R.id.buttonStartStop);
        MaterialButton buttonPauseResume = rootView.findViewById(com.servalabs.cam.R.id.buttonPauseResume);
        ...
        // Update Pause/Resume button (always visible, icon-only like ServaCam)
        if (buttonPauseResume != null) {
            ...
            if (screenRecordingState == ScreenRecordingState.NONE) {
                ...
                buttonPauseResume.setAlpha(0.5f);
                buttonPauseResume.setIcon(
                    AppCompatResources.getDrawable(requireContext(), com.servalabs.cam.R.drawable.pause_rounded)
                );
                ...
                buttonPauseResume.setAlpha(1.0f);
                buttonPauseResume.setIcon(
                    AppCompatResources.getDrawable(requireContext(), com.servalabs.cam.R.drawable.play_button_rounded)
                );
            }
        }
    ...
    private void updateMuteButtonUi() {
        ...
        if (Constants.AUDIO_SOURCE_NONE.equals(audioSource)) {
            iconRes = com.servalabs.cam.R.drawable.ic_volume_off_24;
        } else if (muted && isRecordingActive) {
            iconRes = com.servalabs.cam.R.drawable.ic_volume_off_24;
        } else if (Constants.AUDIO_SOURCE_INTERNAL.equals(audioSource)) {
            iconRes = com.servalabs.cam.R.drawable.ic_volume_up_24;
        } else {
            // Microphone
        ...
        if (sharedPreferencesManager != null) {
            try {
                ...
                boolean serverActive =
                    com.servalabs.cam.streaming.RemoteStreamManager.getInstance().isStreamingEnabled();
     ...
     * {@link com.servalabs.cam.fadrec.services.ScreenRecordingService}, uses
                ...
                if (tvEstimateTitle != null) {
                    ...
                    if (!oldEstimate.equals(finalRemaining)) {
                        if (tvEstimateTitle instanceof com.servalabs.cam.ui.utils.AnimatedTextView) {
                            ...
                            if (formatChanged) {
                                ((com.servalabs.cam.ui.utils.AnimatedTextView) tvEstimateTitle).animateSlotFullDown(finalRemaining, 400);
                            } else {
                                ((com.servalabs.cam.ui.utils.AnimatedTextView) tvEstimateTitle).animateSlotDown(finalRemaining, 400);
                            }
                        } else {
    ...
    protected void updateModeSpecificPreviewVisibility() {
        ...
        View rootView = getView();
        View textureView = rootView.findViewById(com.servalabs.cam.R.id.textureView);
        View flPreviewAvatar = rootView.findViewById(com.servalabs.cam.R.id.fl_preview_avatar);
        View tvPreviewPlaceholder = rootView.findViewById(com.servalabs.cam.R.id.tvPreviewPlaceholder);
        TextView tvPreviewHint = rootView.findViewById(com.servalabs.cam.R.id.tvPreviewHint);
    ...
    private boolean ensurePreviewSurfaceReady() {
        ...
        View maybeTextureView = getView().findViewById(com.servalabs.cam.R.id.textureView);
