...
public class RemoteStreamService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null && "com.servalabs.cam.COPY_STREAM_URL".equals(intent.getAction())) {
        ...
        getSharedPreferences("ServaCamPrefs", MODE_PRIVATE)
            .edit()
    ...
    private Notification buildNotification(String contentText, String streamUrl) {
        ...
        copyIntent.setAction("com.servalabs.cam.COPY_STREAM_URL");
    ...
    private void updateNotification() {
        // Only show if server is running (activePort != -1)
        if (activePort == -1) {
            FLog.d(TAG, "Server not running, skipping notification update");
            return;
        }
        ...
        String ipAddress = getLocalIpAddress();
        String dashboardUrl = "http://" + ipAddress + ":" + activePort + "/";
    ...
    public String getStreamUrl() {
        ...
        // Return local dashboard URL (root)
        String ipAddress = getLocalIpAddress();
