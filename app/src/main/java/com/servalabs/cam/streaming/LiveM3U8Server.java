package com.servalabs.cam.streaming;

import com.servalabs.cam.Log;
import com.servalabs.cam.FLog;
import androidx.annotation.NonNull;

import com.servalabs.cam.streaming.model.ClientEvent;
import com.servalabs.cam.streaming.model.ClientMetrics;
 ...
 */
    ...
    private Response servePlaylist() {
        ...
        if (!streamManager.isStreamingEnabled()) {
            ...
            String message = "❌ Streaming Disabled\n\n" +
                "Please start recording on ServaCam with streaming enabled (STREAM_ONLY or STREAM_AND_SAVE mode).\n\n" +
    ...
    private Response toggleTorch() {
        try {
            ...
            android.content.Intent intent = new android.content.Intent(
                    context, com.servalabs.cam.services.TorchService.class);
            intent.setAction(com.servalabs.cam.Constants.INTENT_ACTION_TOGGLE_TORCH);
    ...
    private Response toggleRecording() {
        try {
            ...
            com.servalabs.cam.SharedPreferencesManager spManager = com.servalabs.cam.SharedPreferencesManager.getInstance(context);
            boolean isRecording = spManager.isRecordingInProgress();
            boolean isPaused = RemoteStreamManager.getInstance().isPaused();
            
            android.content.Intent intent = new android.content.Intent(context, com.servalabs.cam.services.RecordingService.class);
            
            if (isPaused) {
                // Currently paused - resume recording
                intent.setAction(com.servalabs.cam.Constants.INTENT_ACTION_RESUME_RECORDING);
                ...
                // Stop recording
                intent.setAction(com.servalabs.cam.Constants.INTENT_ACTION_STOP_RECORDING);
                FLog.d(TAG, "Recording in progress - sending STOP_RECORDING intent");
            } else {
                // Start recording
                intent.setAction(com.servalabs.cam.Constants.INTENT_ACTION_START_RECORDING);
    ...
    private Response pauseRecording() {
        try {
            ...
            android.content.Intent intent = new android.content.Intent(context, com.servalabs.cam.services.RecordingService.class);
            intent.setAction(com.servalabs.cam.Constants.INTENT_ACTION_PAUSE_RECORDING);
            ServiceStartPolicy.startRecordingAction(context, intent);
    ...
    private Response setVideoCodec(IHTTPSession session) {
        try {
            ...
            editor.putString(com.servalabs.cam.Constants.PREF_VIDEO_CODEC, codec);
            editor.apply();
    ...
    private Response switchCamera(IHTTPSession session) {
        try {
            com.servalabs.cam.SharedPreferencesManager spManager =
                    com.servalabs.cam.SharedPreferencesManager.getInstance(context);
            com.servalabs.cam.CameraType current = spManager.getCameraSelection();
            ...
            com.servalabs.cam.CameraType target;
            if (body != null && !body.isEmpty()) {
                try {
                    org.json.JSONObject json = new org.json.JSONObject(body);
                    String toStr = json.optString("to", "").trim().toUpperCase();
                    com.servalabs.cam.CameraType parsed = null;
                    try { parsed = com.servalabs.cam.CameraType.valueOf(toStr); }
                    catch (IllegalArgumentException ignored) {}
                    target = (parsed != null) ? parsed
                            : (current == com.servalabs.cam.CameraType.BACK
                                    ? com.servalabs.cam.CameraType.FRONT
                                    : com.servalabs.cam.CameraType.BACK);
                } catch (Exception e) {
                    target = (current == com.servalabs.cam.CameraType.BACK)
                            ? com.servalabs.cam.CameraType.FRONT
                            : com.servalabs.cam.CameraType.BACK;
                }
            } else {
            ...
            if (spManager.isRecordingInProgress()) {
                android.content.Intent intent = new android.content.Intent(
                        context, com.servalabs.cam.services.RecordingService.class);
                intent.setAction(com.servalabs.cam.Constants.INTENT_ACTION_SWITCH_CAMERA);
                intent.putExtra(com.servalabs.cam.Constants.INTENT_EXTRA_CAMERA_TYPE_SWITCH, target.toString());
                com.servalabs.cam.utils.ServiceStartPolicy.startRecordingAction(context, intent);
                FLog.i(TAG, "✅ Camera switch (live): " + current + " → " + target);
            } else {
                spManager.sharedPreferences.edit()
                        .putString(com.servalabs.cam.Constants.PREF_CAMERA_SELECTION, target.toString())
                        .apply();
                ...
                android.content.Intent broadcastIntent = new android.content.Intent(
                        com.servalabs.cam.Constants.BROADCAST_ON_CAMERA_SWITCH_COMPLETE);
                broadcastIntent.putExtra(com.servalabs.cam.Constants.BROADCAST_EXTRA_CAMERA_TYPE_FROM,
                        current.toString());
                broadcastIntent.putExtra(com.servalabs.cam.Constants.BROADCAST_EXTRA_CAMERA_TYPE_TO,
                        target.toString());
    ...
    private Response setZoom(IHTTPSession session) {
        try {
            ...
            com.servalabs.cam.SharedPreferencesManager spManager =
                    com.servalabs.cam.SharedPreferencesManager.getInstance(context);
            com.servalabs.cam.CameraType cam = spManager.getCameraSelection();
            ...
            if (spManager.isRecordingInProgress()) {
                android.content.Intent intent = new android.content.Intent(
                        context, com.servalabs.cam.services.RecordingService.class);
                intent.setAction(com.servalabs.cam.Constants.INTENT_ACTION_SET_ZOOM_RATIO);
                intent.putExtra(com.servalabs.cam.Constants.EXTRA_ZOOM_RATIO, ratio);
                if (hasPan) {
                    intent.putExtra(com.servalabs.cam.Constants.EXTRA_PAN_X, requestedPanX);
                    intent.putExtra(com.servalabs.cam.Constants.EXTRA_PAN_Y, requestedPanY);
                }
                com.servalabs.cam.utils.ServiceStartPolicy.startRecordingAction(context, intent);
            ...
            android.content.Intent zoomBcast = new android.content.Intent(com.servalabs.cam.Constants.BROADCAST_ON_ZOOM_CHANGED);
            zoomBcast.putExtra(com.servalabs.cam.Constants.EXTRA_BROADCAST_ZOOM_RATIO, ratio);
            zoomBcast.putExtra(com.servalabs.cam.Constants.EXTRA_BROADCAST_PAN_X, effectivePanX);
            zoomBcast.putExtra(com.servalabs.cam.Constants.EXTRA_BROADCAST_PAN_Y, effectivePanY);
    ...
    private Response setExposure(IHTTPSession session) {
        try {
            ...
            org.json.JSONObject json = new org.json.JSONObject(body);
            com.servalabs.cam.SharedPreferencesManager spMgr = com.servalabs.cam.SharedPreferencesManager.getInstance(context);
            ...
            if (spMgr != null) {
                ...
                android.content.Intent exposureBroadcast = new android.content.Intent(
                    com.servalabs.cam.Constants.BROADCAST_ON_EXPOSURE_CHANGED);
                exposureBroadcast.putExtra(
                    com.servalabs.cam.Constants.EXTRA_BROADCAST_EXPOSURE_COMPENSATION,
                    ev);
            ...
            android.content.Intent intent = new android.content.Intent(
                    context, com.servalabs.cam.services.RecordingService.class);
            intent.setAction(com.servalabs.cam.Constants.INTENT_ACTION_SET_EXPOSURE_COMPENSATION);
            intent.putExtra(com.servalabs.cam.Constants.EXTRA_EXPOSURE_COMPENSATION, ev);
            com.servalabs.cam.utils.ServiceStartPolicy.startRecordingAction(context, intent);
    ...
    private Response setMirror(IHTTPSession session) {
        try {
            ...
            android.content.Intent intent = new android.content.Intent(
                    context, com.servalabs.cam.services.RecordingService.class);
            intent.setAction(com.servalabs.cam.Constants.INTENT_ACTION_SET_FRONT_VIDEO_MIRROR);
            intent.putExtra(com.servalabs.cam.Constants.EXTRA_FRONT_VIDEO_MIRROR_ENABLED, enabled);
            com.servalabs.cam.utils.ServiceStartPolicy.startRecordingAction(context, intent);
    ...
    private Response toggleAeLock(IHTTPSession session) {
        try {
            ...
            android.content.Intent intent = new android.content.Intent(
                    context, com.servalabs.cam.services.RecordingService.class);
            intent.setAction(com.servalabs.cam.Constants.INTENT_ACTION_TOGGLE_AE_LOCK);
            intent.putExtra(com.servalabs.cam.Constants.EXTRA_AE_LOCK, newLocked);
            com.servalabs.cam.utils.ServiceStartPolicy.startRecordingAction(context, intent);
    ...
    private Response handleLogin(IHTTPSession session, String clientIP) {
        try {
            ...
            if (!authManager.isAuthEnabled()) {
                com.servalabs.cam.streaming.model.AuthResponse authResponse = 
                    com.servalabs.cam.streaming.model.AuthResponse.authDisabled();
            ...
            if (body == null || body.isEmpty()) {
                com.servalabs.cam.streaming.model.AuthResponse authResponse = 
                    com.servalabs.cam.streaming.model.AuthResponse.failure("Missing request body");
            ...
            try {
                ...
                com.servalabs.cam.streaming.model.AuthResponse authResponse = 
                    com.servalabs.cam.streaming.model.AuthResponse.failure("Invalid JSON");
                return jsonResponse(Response.Status.BAD_REQUEST, authResponse.toJson());
            }
            ...
            if (!authManager.verifyPassword(password)) {
                ...
                com.servalabs.cam.streaming.model.AuthResponse authResponse = 
                    com.servalabs.cam.streaming.model.AuthResponse.invalidCredentials();
            ...
            com.servalabs.cam.streaming.model.SessionToken token = authManager.createSession(deviceInfo);
            
            FLog.i(TAG, "✅ Successful login from " + clientIP);
            
            com.servalabs.cam.streaming.model.AuthResponse authResponse = 
                com.servalabs.cam.streaming.model.AuthResponse.success(token.getToken(), token.getExpiresAtMs());
            return jsonResponse(Response.Status.OK, authResponse.toJson());
            
        } catch (Exception e) {
            FLog.e(TAG, "Error handling login", e);
            com.servalabs.cam.streaming.model.AuthResponse authResponse = 
                com.servalabs.cam.streaming.model.AuthResponse.failure("Internal error");
    ...
    private Response handleLogout(IHTTPSession session) {
        try {
            ...
            com.servalabs.cam.streaming.model.AuthResponse authResponse = 
                new com.servalabs.cam.streaming.model.AuthResponse(true, "Logged out successfully");
            return jsonResponse(Response.Status.OK, authResponse.toJson());
        } catch (Exception e) {
    ...
    private Response handleAuthCheck(IHTTPSession session) {
        try {
            ...
            if (authEnabled && token != null) {
                com.servalabs.cam.streaming.model.SessionToken sessionToken = authManager.validateToken(token);
    ...
    private Response handleChangePassword(IHTTPSession session) {
        try {
            ...
            if (!validateAuthToken(session)) {
                com.servalabs.cam.streaming.model.AuthResponse authResponse = 
                    com.servalabs.cam.streaming.model.AuthResponse.unauthorized();
                return jsonResponse(Response.Status.UNAUTHORIZED, authResponse.toJson());
            }
            ...
            if (!authManager.verifyPassword(oldPassword)) {
                com.servalabs.cam.streaming.model.AuthResponse authResponse = 
                    com.servalabs.cam.streaming.model.AuthResponse.failure("Current password is incorrect");
                return jsonResponse(Response.Status.UNAUTHORIZED, authResponse.toJson());
            }
            ...
            if (!authManager.setPassword(newPassword)) {
                com.servalabs.cam.streaming.model.AuthResponse authResponse = 
                    com.servalabs.cam.streaming.model.AuthResponse.failure("New password invalid (4-32 characters required)");
                return jsonResponse(Response.Status.BAD_REQUEST, authResponse.toJson());
            }
            
            FLog.i(TAG, "Password changed successfully");
            com.servalabs.cam.streaming.model.AuthResponse authResponse = 
                new com.servalabs.cam.streaming.model.AuthResponse(true, "Password changed successfully");
            return jsonResponse(Response.Status.OK, authResponse.toJson());
    ...
    private Response ringAlarm(IHTTPSession session) {
        try {
            ...
            android.content.Intent alarmIntent = new android.content.Intent(context, com.servalabs.cam.services.AlarmService.class);
            alarmIntent.setAction("com.servalabs.cam.action.RING_ALARM");
    ...
    private Response stopAlarm() {
        try {
            ...
            android.content.Intent alarmIntent = new android.content.Intent(context, com.servalabs.cam.services.AlarmService.class);
            alarmIntent.setAction("com.servalabs.cam.action.STOP_ALARM");
            context.startService(alarmIntent);
    ...
    private Response handleNotifications(IHTTPSession session) {
        try {
            ...
            com.servalabs.cam.SharedPreferencesManager prefs = com.servalabs.cam.SharedPreferencesManager.getInstance(context);
    ...
    private Response fetchGitHubNotification() {
        try {
            String githubUrl = "https://raw.githubusercontent.com/anonfaded/ServaCam/master/app/src/main/assets/web/fadex/pushnotification.jsonc";
            ...
            conn.setRequestProperty("User-Agent", "ServaCam-NotificationManager");
