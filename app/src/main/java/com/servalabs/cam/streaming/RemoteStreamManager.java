...
import com.servalabs.cam.streaming.model.NetworkHealth;
import com.servalabs.cam.streaming.model.StreamQuality;
import com.servalabs.cam.streaming.util.NetworkMonitor;
...
public class RemoteStreamManager {
    ...
    public void setStreamingEnabled(boolean enabled) {
        ...
        try {
            ...
            if (enabled) {
                ...
                if (context != null) {
                    android.content.SharedPreferences prefs = context.getSharedPreferences("ServaCamPrefs", android.content.Context.MODE_PRIVATE);
                    prefs.edit()
    ...
    public String getStatusJson() {
        ...
        try {
            ...
            com.servalabs.cam.CameraType activeCam = spMgr != null ? spMgr.getCameraSelection() : com.servalabs.cam.CameraType.BACK;
            ...
            String result = String.format(java.util.Locale.US,
                ...
                streamingEnabled,
                com.servalabs.cam.streaming.util.JsonEscaper.escapeToJsonString(streamingMode.toString().toLowerCase()),
                com.servalabs.cam.streaming.util.JsonEscaper.escapeToJsonString(state),
                com.servalabs.cam.streaming.util.JsonEscaper.escapeToJsonString(message),
                lastUpdatedTimestamp,
                com.servalabs.cam.streaming.util.JsonEscaper.escapeToJsonString(serverVersion),
                isRecording,
                recordingPaused,
                bufferedCount,
                ...
                fragmentSequence,
                oldestSequence,
                getAllClientMetrics().size(), // activeConnections: local clients count
                hasInit,
                uptimeSeconds,
                batteryDetailsJson,
                uptimeDetailsJson,
                com.servalabs.cam.streaming.util.JsonEscaper.escapeToJsonString(networkType),
                isNetworkConnected,
                networkHealthJson,
                qualityJson,
                com.servalabs.cam.streaming.util.JsonEscaper.escapeToJsonString(codecName),
                com.servalabs.cam.streaming.util.JsonEscaper.escapeToJsonString(cameraTypeName),
                zoomRatio,
                zoomRatioMin,
                zoomRatioMax,
                panX,
                ...
                volumePercentage,
                alarmRinging,
                com.servalabs.cam.streaming.util.JsonEscaper.escapeToJsonString(selectedAlarmSound),
                alarmDurationMs,
                ...
                com.servalabs.cam.streaming.util.JsonEscaper.escapeToJsonString(memoryUsage),
                com.servalabs.cam.streaming.util.JsonEscaper.escapeToJsonString(storageInfo),
                totalDataMB
            );
            ...
            return "{\"streaming\": " + streamingEnabled + ", \"state\": \"error\", \"message\": \"JSON generation failed\", \"error\": \"" + 
                   com.servalabs.cam.streaming.util.JsonEscaper.escapeToJsonString(e.getMessage()) + "\"}";
        } finally {
            bufferLock.readLock().unlock();
        }
    }
    ...
    public boolean isTorchOn() {
        if (context != null) {
            try {
                ...
                boolean torchStateFromPrefs = prefs.getBoolean(com.servalabs.cam.Constants.PREF_TORCH_STATE, false);
    ...
    public String getBatteryDetailsJson(android.content.Context context) {
        ...
        String warningJson = warning.isEmpty() ? "\"\"" : com.servalabs.cam.streaming.util.JsonEscaper.escapeToJsonString(warning);
    ...
    /**
     * Get number of viewers connected via cloud relay.
     */
    public int getCloudViewerCount() {
        // Placeholder for cloud viewer count logic
        return 0;
    }
}